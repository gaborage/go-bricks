...
| Messaging publisher pre-warm (startup) | `messaging.reconnect.readytimeout` | 5s | Single-tenant startup waits for the pre-warmed publisher to report ready, reusing the same key and budget; WARN and continue on timeout — never fails startup. See [startup_defaults.md](startup_defaults.md#messaging-pre-warm-readiness-wait) |
| AMQP publish (call bound) | `readytimeout + maxpublishattempts × connectiontimeout`, unless `messaging.publishtimeout` caps it | **150s warm / 155s ceiling** | Timeout-path worst case of one `Publisher[T].Publish` at default backoffs: 5 × 30s confirmation waits (warm client; pre-flight is ~0) plus up to 5s `messaging.reconnect.readytimeout` when the client is cold or reconnecting. Each attempt's ACK/NACK wait is `messaging.reconnect.connectiontimeout` (30s). A raised `reconnect.resenddelay` can stretch the publish-error path — see [messaging.md](messaging.md#bounded-publish-retries-reconnectmaxpublishattempts). Connection dial uses `amqp091-go`'s `amqp.Dial` (its own ~30s TCP+handshake timeout), not this key. |
| AMQP publish aggregate bound | `messaging.publishtimeout` | **unset (unbounded)** | Opt-in ceiling on one whole publish — readiness pre-flight plus the entire retry loop — applied as a derived context deadline, so the effective bound is the shorter of it and the caller's deadline. Unset or 0 reproduces the row above exactly. Startup refuses a value below `readytimeout + connectiontimeout` (it could truncate a healthy first attempt into a false failure) and any negative value. A value below `maxpublishattempts × connectiontimeout` deliberately lowers the effective retry count. It bounds **waiting** — the readiness pre-flight, the serialized publish slot, and the retry loop's confirmation and backoff waits, all observed under the context — never an in-flight socket write, so a broker that stops reading holds only the one publish already inside the write past the bound, not the publishers queued behind it. See [messaging.md](messaging.md#aggregate-publish-bound-publishtimeout). |
...
| Background job step inside a scheduler `Executor` | **per-job** | Scheduler doesn't cancel slow jobs — it logs at `slowjob` (25s) and reports duration. Apply your own deadline if the step has a known SLO. Size a publish step against the real AMQP call bound (`readytimeout + maxpublishattempts × connectiontimeout` — 150s warm / 155s ceiling at defaults), not the 30s per-attempt confirmation wait; see [messaging.md](messaging.md#bounded-publish-retries-reconnectmaxpublishattempts). Deadline-free scheduler jobs are exactly what `messaging.publishtimeout` exists for: setting it bounds every publish deployment-wide, without each job having to derive its own deadline. |
