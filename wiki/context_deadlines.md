...
| AMQP publish (call bound) | `readytimeout + maxpublishattempts × connectiontimeout` | **150s warm / 155s ceiling** | Worst-case wall clock of one `PublishToExchange` at defaults: 5 × 30s confirmation waits (warm client; pre-flight is ~0) plus up to 5s `messaging.reconnect.readytimeout` when the client is cold or reconnecting. Each attempt's ACK/NACK wait is `messaging.reconnect.connectiontimeout` (30s). Connection dial uses `amqp091-go`'s `amqp.Dial` (its own ~30s TCP+handshake timeout), not this key. See [messaging.md](messaging.md#bounded-publish-retries-reconnectmaxpublishattempts) |
