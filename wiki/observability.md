...
- **Shutdown races an export it cannot cancel.** Two different durations, easy to merge and worth keeping apart. `Provider.Shutdown(ctx)` returns when YOUR context expires — it does not block past it. But an export the batch processor's timer already started runs on a context of the SDK's own, never derived from yours, bounded only by that signal's own `observability.<signal>.export.timeout`; `Shutdown` races it and, against an unreachable collector, loses. So it returns `context deadline exceeded` while the dial keeps retrying in the background after you were told shutdown finished. The teardown is best-effort: the failure is one WARN line (`Observability shutdown failed; telemetry may have been lost`) and telemetry still queued past the budget is dropped, but the phase never contributes to the error `App.Run()` returns, so a collector that is down cannot fail a graceful shutdown ([ADR-029](adr_029_graceful_shutdown_order.md) amendment). `Provider.Shutdown` covers traces, metrics and logs, each with its own timeout, so size the deployment's shutdown budget above the LARGEST `observability.<signal>.export.timeout` among the signals you enable — sizing it against the trace timeout alone leaves metrics or logs past the budget — or lower those timeouts: with the 10s default — `observability.environment: development`, or any signal whose endpoint is `stdout` — and a shorter budget, that WARN is how you learn telemetry was lost; sizing the budget above the export timeout is how you KEEP it
...
`/ready` is one framework-owned judgment ([ADR-066](adr_066_readiness_one_module.md)): it walks the four resource slots (`database`, `messaging`, `cache`, `streams`), answers 503 for the first component that is both failing and critical, and renders each kind's status plus an allowlisted subset of its statistics on the 200 body, with error text sanitised by default ([ADR-048](adr_048_ready_sanitize_by_default.md)). There is no registration door for a consumer-supplied probe: `app.Prober` is implemented by the framework's own probe descriptions and nothing else. The reasoning and the reopen triggers live in `.out-of-scope/readiness-contribution-door.md`.

A service with a non-framework dependency that must gate traffic has one path today: `Server.RegisterReadyHandler` replaces the framework's `/ready` handler. It is a replacement, not an extension. The handler you install loses the slot walk, the critical gate, the public-stats allowlist and the sanitised error bodies, and must re-implement whichever of those it still wants. Prefer expressing the dependency as framework configuration where one exists (`cache.critical`, and the opt-in consumer-subscription criticality tracked in #1666) before reaching for the replacement. Either handler sits behind a shutdown gate: once shutdown begins, `/ready` answers `503` `{"status":"not ready"}` instead of running the registered handler. With `server.probes.port` set, the probe listener adds two more gates before the handler runs — the application listener must be serving and must answer the application-listener check ([startup_defaults.md](startup_defaults.md#internal-probe-listener)). Concurrent `/ready` requests share one in-flight framework judgment, but an override is not coalesced: it runs once per request, so one that reaches a backend bounds its own cost.
...
The same field-name limit applies to SQL: the database tracking wrapper's `query` log field and the `db.query.text` span attribute carry statement text the filter cannot see into, so two clause shapes — a PostgreSQL `PASSWORD '<literal>'` and an Oracle `IDENTIFIED BY <password>` — are cut at the keyword by `database/sqlredact`, before truncation on both paths, with the value and everything after it replaced by `[REDACTED]`. A password inside a connection-string literal is not recognized, and no other statement-embedded literal is scrubbed at all, so keep values in bind parameters.
...
| Key material | `api_key`, `apikey`, `api-key`, `private_key`, `privatekey`, `private-key`, `signing_key`, `signingkey`, `signing-key`, `encryption_key`, `encryptionkey`, `encryption-key`, `keyvalue` |
