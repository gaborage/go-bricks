...
The same field-name limit applies to SQL: the database tracking wrapper's `query` log field and the `db.query.text` span attribute carry statement text the filter cannot see into, so credential-bearing DDL (PostgreSQL `PASSWORD`, Oracle `IDENTIFIED BY`) is cut at the keyword by `database/sqlredact`, before truncation on both paths, with the value and everything after it replaced by `[REDACTED]` — other statement-embedded literals are not, so keep values in bind parameters.
