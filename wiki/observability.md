...
- **Scoped to `Err`.** A nil error still emits nothing and never reaches the redactor. `Interface`, `WithFields` and `Msgf` are unchanged — an error logged through those goes through field-name masking only. Recovered panic values are governed by ADR-081 (reported by type, never by value) and are not routed here. Read that precisely: the panic VALUE never reaches the redactor, but the server's recovered-panic log line does hand its ADR-081 type-rendered error to `Err` under `app.debug` (#1182), so a redactor sees `panic (type: string)` — the framework's own rendering — and not the value the handler panicked with.
