...
A service with a non-framework dependency that must gate traffic has one path today: `Server.RegisterReadyHandler` replaces `/ready` wholesale. It is a replacement, not an extension. The handler you install loses the slot walk, the critical gate, the public-stats allowlist and the sanitised error bodies, and must re-implement whichever of those it still wants. Prefer expressing the dependency as framework configuration where one exists (`cache.critical`, and the opt-in consumer-subscription criticality tracked in #1666) before reaching for the replacement.
