...
> This amendment's change is `[C65.11]`, a runtime refusal: no signature moves.
>
...
> amendment ([C65.11]). The §4 `^[A-Za-z0-9_-]{1,128}$`
...
framework-minted, carries its `:` on purpose, and `inbox.ProcessOnce` admits it only when it
equals the key the sealed door stored on that delivery's context (`messaging.IsSealedDelivery`
is true iff such a key is present), so the two key
