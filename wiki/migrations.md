...
- gist: under a sealed delivery's context `ValidateDedupKey` admitted any sealed key, whichever
  ...
  stamped, and a sealed key is admitted only when it equals that one — a key retained from delivery A
  is refused while handling delivery B. Wire keys stay unbound (C66.2, ADR-097 amendment, #1634).
...
- detect: nothing in your build flags this — no signature moves. `git grep -nE
  ...
  when the key it passes is SEALED and did not come from the delivery being handled.
...
- scope: `messaging.ValidateDedupKey(ctx, key DedupKey)` still refuses the zero key
  ...
  stream declaration).
- gate: match — a `ProcessOnce` call passes a SEALED key that another delivery minted:
  one retained across deliveries, or one read from a sibling delivery's `meta`.
  no-match — every sealed key a handler passes is the current delivery's own
  `meta.DedupKey()`, or the key is a `WireDedupKey`, which this atom does not touch.
