...
| E65 | v0.64.0 → v0.65.0 | breaking (C65.1 — under an `httpclient` JOSE `Inbound` policy, a 2xx response the transport did not unwrap is refused as `httpclient.ErrJOSEPlaintextResponse` instead of reaching the caller as plaintext: in nested mode a successful status whose `Content-Type` is not `application/jose`, in envelope mode one whose `Unwrap` declined the body; the body is closed and never handed to the caller or to a response interceptor, and non-2xx pass-through, 204/304/HEAD and every crypto failure are unchanged) | 1 | none — `JOSETransport` and `JOSEConfig` each GAIN a bool field, so every existing literal, keyed or positional, still compiles; the change is a runtime refusal on a response path | inventory the peers you reach through JOSE with `git grep -n 'WithJOSE(' -- '*.go'` and again with `git grep -n 'JOSETransport{' -- '*.go'` (two runs, because an alternation cannot live in a table cell) and, for each, establish from the partner spec or from a capture whether EVERY 2xx route returns a protected body — a health check, a redirect-shaped 2xx, a 202 acknowledgement or a legacy route half-migrated to MLE is the population that breaks; set `AllowPlaintextSuccess: true` on exactly those integrations BEFORE the bump and clear it once the peer protects every route (C65.1) |
...
## E65 · v0.65.0 — a successful JOSE response that was never unwrapped is a transport error
...
- gist: `JOSETransport` passed every body it did not recognize straight through. In nested
  ...
  knob for a peer that still answers some 2xx routes in plaintext (C65.1, ADR-107 amendment).

### [C65.1] an unwrapped 2xx JOSE response is refused instead of passed through · breaking · when: match
