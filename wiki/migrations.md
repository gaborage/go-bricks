...
- detect: the population is every PostgreSQL section with no `connectionstring` any of whose
  comma-separated `host` entries starts with `/`, or with an uppercase drive letter and `:\`,
  AND whose `database.tls` sets `ca`, `cert` or `key`, or a `mode` other than `disable`; a host
  with an empty entry (`db.internal,`) is refused as `[C64.8]` regardless. Static config:
...
- scope: `validatePostgreSQLFields` gains a rule between the mode allowlist and the
  material/mode coherence check. When any entry of the trimmed `host`, split on `,` exactly as
