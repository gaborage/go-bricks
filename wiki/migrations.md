...
- scope: `NewPostgresStore` and `NewOracleStore` judge the table name's schema segment
  ...
  `sqlid.ValidateTableName` grammar admits, which judges every vendor at Oracle's 128. The
...
- apply: rename the schema to 63 bytes or fewer BEFORE the bump. The name was never usable
  ...
  there. This one fails at startup, not at a query, so it takes the deployment down rather
  than one statement.
- verify: `go build ./... && go test ./...`  # then boot the service against a PostgreSQL
  outbox and confirm startup is green with the renamed schema; on Oracle confirm a
  128-byte schema still boots
