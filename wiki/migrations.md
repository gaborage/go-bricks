...
- detect: ask the SERVER, because both names are normally derived from a tenant id at
  ...
  `migration/roles_integration_test.go`. The lone
  exception is the multi-statement `REVOKE` script under **apply**, whose semicolons separate
  its five statements. Start with the schema names themselves — is one of these a schema YOU
  ...
  **Deciding the schema half.** Ownership is not the fingerprint, and `\dn+` will not tell you
  ...
  `pg_roles` drops. As with the grant query, a row is not proof: a deliberate `ALTER DEFAULT
  ...
  **Second detect step — case variants the server ACCEPTED.** PostgreSQL's reserved-name check
  ...
  ever handed to the PUBLIC pseudo-role.
  ...
  **Fourth detect step — what a NAMED role still holds on `public`.** The three queries above
  ...
  sequence is a `pg_class` row here exactly as it is there. If you have already RESET the
  `search_path` of a role, this query can no longer find it: run this step BEFORE that part of
  apply, or substitute the role name by hand. A row is not proof on its own: an object's owner
  carries an implicit ACL entry once any grant is made, so a candidate role that also owns
  objects in `public` reports its own ownership privileges here.
...
- apply: (a) fix the names BEFORE the bump; the framework will not do it for you, because
  moving a populated schema is a data operation. Schema: `ALTER SCHEMA "public" RENAME TO
  tenant_a` moves the objects in one statement. Role: there is nothing to rename — the server
  never created a role called `public` or `pg_…`, it refuses both (`reserved_name`, 42939) —
  so change the spec's field to a real name and undo what the PUBLIC grants handed out. One
  revoke per `source` the detect query reported, and they are not interchangeable — the
  first clears a `relation` row, the second another `relation` row (sequences are `pg_class`
  rows too), the third the `schema` row, and the last two the `default_acl` rows, which no
  `REVOKE … ON ALL …` touches because they govern objects that do not exist yet:
  ...
  The `FOR ROLE` must name the same `MigratorRole` that granted them, or the revoke silently
  targets a different default-ACL row and the `default_acl` finding survives.

  Those five revoke nothing the fourth detect step reports, because that step's rows were
  granted to a real role and not to PUBLIC. Run the fourth query FIRST (a `search_path` reset
  below erases the residue it derives its candidates from), then, for every runtime role it
  named — this is the live exposure, so it is not optional:
  ...
  provisioning time. Revoke only what the role should not keep: if the tenant genuinely shares
  a reference table in `public`, re-grant that one afterwards. Then rerun
  provisioning with the new names so both
  roles' `search_path` and the default privileges follow, and repoint any application
  connection, credential or `flyway.conf` that named the old one. A schema genuinely shared
  by design is not a tenant schema — provision it outside this helper. (b) Nothing to do.
- verify: rerun your provisioning call and check it returns nil; the refusal is
  ...
  moved with `SELECT schemaname, count(*) FROM pg_tables GROUP BY 1`, and rerun the detect
  query above — it is the verify step too. It must return no row for a tenant schema under
  any `source`: `schema` gone means the schema `REVOKE` landed, `relation` gone means both
  `ON ALL …` revokes landed, `default_acl` gone means the `ALTER DEFAULT PRIVILEGES …
  REVOKE` named the right `FOR ROLE`. A `relation` row for a reference table you share on
  purpose is expected and stays, and so is the baseline `schema | public | | USAGE` row.
  Rerun the FOURTH detect query too, before you reset either role's `search_path` — it is a
  verify step as much as a detect one, and it is the only one that answers whether the runtime
  role still holds DML on `public`. It must return no row for a role of yours under either
  `source`, with the owner's own implicit ACL entry the one expected exception. Once the
  `search_path` reset has landed the query derives no candidates at all and reads clean
  vacuously, so its verdict is only worth anything while that residue is still there.
