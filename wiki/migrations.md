...
- detect: `git grep -nE '\.Args\[|\.Args[[:space:]]*=|(^|[^A-Za-z0-9_])Args:[[:space:]]*[^[:space:]]' -- '*.go'` then keep hits that populate a `QueueDeclaration`/`ExchangeDeclaration`/`BindingDeclaration`'s `Args` field — indexed writes (`q.Args[k] = v`), whole-map assignment (`q.Args = map[string]any{...}`), or struct-literal fields, directly or via `decls.Queues[...]`/`decls.Exchanges[...]`/`decls.Bindings[...]`. The struct-literal branch deliberately matches **any** value, not just a `map[...]` literal, so `Args: args`, `Args: nil`, and `Args: make(map[string]any)` are caught too; the leading `(^|[^A-Za-z0-9_])` keeps it from firing on unrelated identifiers ending in `Args:` (e.g. `expectedArgs:` in tests). Expect unrelated `Args` fields on non-declaration types in the results — that is what the "keep hits that populate" filter above is for
