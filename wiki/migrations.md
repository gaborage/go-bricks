...
## E64 · v0.63.0 → v0.64.0 — both `SetMap` doors order their columns by the caller's names and the JoinFilter family spells inequality `<>`
...
- gist: `InsertQueryBuilder.SetMap` sorted the caller's column names and quoted in that
  ...
  aligned by `[C61.11]` (C64.1, ADR-099).
