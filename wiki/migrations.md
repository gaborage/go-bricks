...
- apply: bound the value at ITS source rather than at the publish — a routing key is a routing
  ...
  writable. The outbox relay does NOT rescue you here, and the previous wording of this clause
  said the opposite. The relay classifies exactly ONE thing as poison — headers it cannot decode,
  which dead-letter at `MaxRetries` — and treats EVERY broker-side publish failure as connectivity,
  `ErrInvalidPublishDestination` included (`outbox/relay.go`, `publishRecord`). So such an event
  advances `retry_count`, stays `pending`, and is retried on every cycle FOREVER; it never parks.
  A destination your own code can emit but the broker can never accept is therefore a permanent
  item in the relay's working set, which is the second reason to bound the value at its source
  rather than to lean on the relay.
