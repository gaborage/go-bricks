...
- gist: The `go-bricks-migrate` CLI now validates every tenant's resolved database config the
  ...
  access-log field; and the classic AMQP lane's `CorrelationId`, `MessageId`,
  `RoutingKey` and `Exchange`, which live in the delivery's content-header properties and
  `basic.deliver` envelope where no header extractor reaches them and landed verbatim in log
...
- verify: three probes against a running service; the third is broker-dependent.
  ...
  2. **AMQP properties.** Publish to a consumed queue with `correlation_id` set to
