...
- preflight: if you run `go-bricks-migrate`, `go-bricks-migrate info` per tenant before the bump
  ...
  (C60.17); and grep every deployment surface for `DEBUG_TRUSTEDPROXIES` and
  `SCHEDULER_SECURITY_TRUSTEDPROXIES` holding `0.0.0.0/0` or `::/0`, which now fail startup
  the way `server.trustedproxies` always did, plus `debug.allowedips` for an entry that is
...
### [C60.21] the log filter walks JSON arrays without panicking, and normalizes its needle list · breaking · when: match

- detect: three independent checks, run all three.
  ...
  (2) **The emitted type.** `git grep -nE "FilterValue\(" -- '*.go'` and check whether any
  hit type-asserts the result to a concrete slice type
  (`.([]string)`, `.([]MyStruct)`, `.([]any)`) — that is the only way this change is
  observable, and only for slices whose elements the filter rewrites.
  ...
  In YAML,
  `git grep -nE "^[[:space:]]*-[[:space:]]*((''|\"\")?)[[:space:]]*$" -- '*.yaml' '*.yml'`
  finds an empty entry under `log.sensitivefields` in all three spellings — `- ''`, `- ""`,
  and a bare `-`. Do not reach for a `\x27` escape for the single quote: `git grep -E` is
  POSIX ERE, which does not honour it, and the pattern then silently skips `- ''` — the
  ...
  `git grep -nE "json\.RawMessage|^type [A-Za-z_][A-Za-z0-9_]* \[\]byte" -- '*.go'`.
  Then list the log-field calls:
  `git grep -nE "\.(Interface|WithFields)\(" -- '*.go'`.
...
- scope: three changes in the sensitive-data filter, all consumer-visible.
...
- gate: match on ANY of — (a) you log decoded JSON, request/response bodies, broker payloads
  ...
  `LogPayloads`; (b) you call the public `FilterValue` and type-assert its result to a
  concrete slice type; (c) you build a needle list in Go via
  `app.Options.LoggerFilterConfig` or `logger.NewSensitiveDataFilter`, or in YAML via
  `log.sensitivefields`, and it can contain an empty, whitespace-only or duplicate entry.
  no-match = you log only scalars and typed structs, never assert on `FilterValue`'s result,
  and your needle lists are literal and clean. Note (a) is the common case and its old
  behaviour was a crash, so matching it is good news, not work.
- apply: nothing to write for (a) — the crash stops. For (b), change the assertion to accept
  `[]any`, or read the value through JSON rather than through a type assertion; there is no
  longer any input for which the emitted type depends on whether a needle happened to match,
  so an assertion that was passing by luck will now fail consistently rather than
  intermittently. For (c), remove the empty entry — but read the next line first, because
...
- verify: log a document shaped `{"data":[{"password":"p","name":"n"}]}` through
  `Interface()` and again through `WithFields()`. Both must emit
  `{"data":[{"password":"***","name":"n"}]}` — masked INSIDE the array element, which proves
  the walk reaches in rather than merely not crashing. Then, if you build needle lists in
  Go, construct your real `FilterConfig` and log a two-field document: a field your list does
  NOT name must appear in clear. If it comes out masked, your list still carries an entry
...
- detect: `git grep -nE 'trustedproxies' -- '*.yaml' '*.yml' '*.json' '*.toml'` and the same
  ...
  rendered manifests) for `DEBUG_TRUSTEDPROXIES` and `SCHEDULER_SECURITY_TRUSTEDPROXIES`.
  **A `0.0.0.0/0` or `::/0` entry in either now fails startup**, and so does a list whose
