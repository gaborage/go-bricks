...
## E61 · v0.60.0 → v0.61.0 — the server's own 400 details stop echoing request input, and `Having` gains a sanctioned expression path
...
- apply: for equality, nothing — the new rendering is what the call always meant; if you worked
  ...
  wrapper, which now reads as the resolved value. If a `jf` call site was crashing on a nil
  pointer to a Valuer type, drop the nil guard you wrote around it; if the SAME operand also
  reaches `f`, `jf.In`, `jf.NotIn` or `jf.Between`, keep that guard — those still panic.
- verify: `jf.Eq("u.id", nil)` and `jf.Eq("u.id", sql.NullString{})` both render `u.id IS NULL`;
  ...
  the panic, not only on the SQL. Note `jf.Between` reports squirrel's
