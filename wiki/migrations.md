...
| E581 | v0.58.0 → v0.58.1 | silent-behavior | 2 | none | if you cache any type carrying a time.Time, decide before the bump whether a compare-and-set on a sub-second timestamp may fail during the rolling deploy (C581.1); and if `observability.logs.samplingrate` is set to at least 0.00005 and below 0.01, expect the exported INFO/DEBUG log volume to change — that rate exported nothing before the bump and starts exporting its configured fraction after it; a rate below 0.00005 still exports nothing (C581.2) |
...
- gist: The cache's CBOR encoder moves from `cbor.TimeRFC3339` to
  ...
  still rounds to zero and still exports nothing.
- build-caught: none
- preflight: if any cached type carries a `time.Time` and you use
  ...
  `observability.logs.samplingrate` is set to at least 0.00005 and below
  0.01, expect the exported INFO/DEBUG log volume to change — that rate
  exported nothing before the bump and starts exporting its configured
  fraction after it; a rate below 0.00005 still exports nothing
