...
- preflight: if you run `go-bricks-migrate`, `go-bricks-migrate info` per tenant before the bump
  ...
  Then check `debug.allowedips` for an entry that is
  neither an IP address nor a CIDR range — and note the ALLOWLIST keys may still legitimately
  hold a default route (C60.22)
...
- scope: [C60.16] addressed a database section's errors to that section at the STARTUP doors
  ...
  `databases.report_db.port` (that those names are unreachable at all is #1124, not fixed
...
- detect: FIVE log lines change shape across four packages — **plus the SPAN text on both
  messaging consume spans and the scheduler's job span, which no log grep reaches** — and
  every one breaks a matcher SILENTLY: the query stops
  matching and the alert simply never fires, which reads as "no panics" rather than as a
  broken alert. Grep your log-based alerts, saved queries, dashboards and log-parsing tests
  for all five: the audit sink-failure line (`audit sink panicked; event dropped`), the
  ...
  them — **the shared delivery outcome line on BOTH messaging lanes**, whose classic
  spelling is `Panic recovered in message handler - discarding without requeue` and whose
  streams spelling the runner supplies per call. That last pair is the one to check first:
  it fires for any consumer whose MESSAGE HANDLER panics, which needs no `AuditRecorder`
  ...
  **Then run the span-side detect, which the log grep above cannot substitute for.** In your
  ...
  `panic in message handler` and for `cleanup: tenant`, and grep alert and SLO definitions
  ...
  fully affected.
- scope: six packages report a recovered panic value, and after this hop none of them
  carries the value itself — **on any sink, not only the log**. **The one that reaches every
  ...
  chosen by consumer code. The stack trace is retained wherever it was emitted before — audit, scheduler and the delivery
  outcome line; the settle line never carried one — and `audit_type` /
  `target` / `jobID` still make each report attributable. The audit emitter's second message,
...
- gate: match on ANY of — (a) an alert, saved query, dashboard or log-parsing test reads the
  `panic` field, or matches on a panic VALUE, on ANY of the five lines — including a
  ...
  (e) anything reads the `error` field on the HTTP action log line, or an HTTP server span's
  status description, for a panicking request — **this is the broadest clause in the atom and
  needs no messaging, scheduler or audit usage to apply**: it fires for any service whose
  handler can panic; (b) you match the literal
  ...
  nothing you own reads the `panic` field or the message text of ANY of the five lines — audit
  sink-failure, scheduler job-panic, delivery settle, and the shared delivery outcome line on
  both messaging lanes — you have no lane-shape test pinning the delivery spine's key set,
  **nothing reads `exception.message` or the span status description on a messaging consume
  span or a scheduler job span**, and no runbook depends on the panic value being present. **Running no scheduled jobs and
  registering no `AuditRecorder` is NOT enough to reach no-match**: the messaging pair fires
  whenever a consumer's message handler panics.
- apply: for (a), repoint at `panic_type` and at the stack trace, which is retained and
  identifies the panic site. For (b), delete the matcher; that message has no successor.
...
- verify: for (a), in staging make an `AuditRecorder.Record` panic with a synthetic secret
  ...
  description, and both must read `panic (type: string)`. **Check the span even if the log
  check passed** — they changed for
  different reasons and a log-only probe cannot tell you the span was fixed.
- ref: [ADR-081](adr_081_recovered_panic_values_reported_by_type.md) ·
  ...
  `migration/audit_emitter.go` · `scheduler/module.go`
...
- detect: `git grep -nE 'trustedproxies' -- '*.yaml' '*.yml' '*.json' '*.toml'` and the same
  ...
  **A `0.0.0.0/0` or `::/0` entry in the first two now fails startup**, and so does a list whose
  ...
  Then check `debug.allowedips` for an entry that is neither an IP address nor a CIDR
  range — a typo there used to deny everything silently at runtime and now names itself at
  startup.
