...
- scope: `database/internal/builder/` only — a shared helper `requireConflictColumnsInInsertSet`
  ...
  rejection**, so only PostgreSQL call sites change behavior. Membership keys on vendor identity, the
  same rule the checks around it use — see C59.9, which brought them into line. Grep alone
