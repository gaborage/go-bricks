...
- preflight: **four** actions. (i) If a proxy in front of the service sits on
  ...
  DO NOTHING insert with an explicit `UPDATE` in the same transaction. If they
  ...
  and keyed on the conflict columns, since one atomic statement becomes two
  and a concurrent writer can otherwise interleave between them — and do
  not silently drop the column: that would keep the old key value and lose
  a write.
...
- after: both vendors return an error from the builder before any SQL is produced. Only PostgreSQL
  ...
  explicit `UPDATE` in the same transaction. If it differs, the call was **rewriting the conflict column** on a matched
