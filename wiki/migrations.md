...
- scope: four columns join the ledger — `seq` (a per-ledger identity assigned at insert),
  ...
  `outbox.superstreams` and `OutboxEvent.Stream` arrive with the lane column but the relay does
  not yet publish to a super stream, so a non-empty `outbox.superstreams` is REFUSED at startup
  on this hop.
...
- detect: `git grep -nE 'app\.StreamDeclarer|ModuleRegistry.*DeclareStreams|messaging/streams' -- '*.go'`
  and `git grep -nE 'messaging\.streams\.uri|MESSAGING_STREAMS_URI' -- '*.yaml' '*.yml' '*.env' '*.go'`
  Hits on `app.StreamDeclarer` or a direct `DeclareStreams` call on the registry are compile
  breaks. Hits on `messaging/streams` mean you already link the lane. Hits on the URI with
  no `messaging/streams` import in the same module are the new startup failure.
- scope: `app` no longer imports `messaging/streams`. The lane is present if and only if
  ...
  vendor client.
- gate: match = you import `app.StreamDeclarer`, call `ModuleRegistry.DeclareStreams`,
  set `messaging.streams.uri`, or implement `DeclareStreams`. no-match = you never
  configured native streams and never named those types.
...
- verify: `go build ./...` is clean. A process with the URI and the import starts (or
  fails for a broker reason, not `ErrStreamsNotLinked`). A process with neither starts
  clean. `go list -deps` of a core-only consumer (`app`, `inbox`, `messaging`, …) does
  not name `rabbitmq-stream-go-client`, `snappy`, `pierrec/lz4`, `pkg/errors` or `murmur3`.
