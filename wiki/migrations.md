...
- scope: `server.ClientIP` derives the address for the framework's two IP-based
  ...
  still does not disable the whole list; only the default route is newly refused.
...
- verify: three probes against a running service.
  ...
  3. **`/_sys/job`.** Same two requests against the scheduler endpoint with the shipped
     ...
     request from an allowlisted address still succeeds — this change can only deny where
     it previously granted wrongly, so a legitimate caller being denied means your
     trusted-proxy list no longer describes your topology.
- ref: [ADR-080](adr_080_client_ip_answers_only_from_observed_hops.md) ·
  ...
  is retired in an addendum) · [ADR-049](adr_049_debug_endpoints_fail_closed.md) (never
  examined IP derivation, which is why this survived it) · `server/clientip.go` ·
  `config/validation.go` (`rejectDefaultRoute`, `validateIPOrCIDRList`)
- residual: `["0.0.0.0/1","128.0.0.0/1"]` covers all of IPv4 with two properly-masked,
  non-default-route entries and passes this rule unchanged. A union-coverage check is a
  follow-up. With the walk fixed it no longer yields a caller-authored answer, only a
  peer-derived one.
