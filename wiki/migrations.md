...
## E65 · v0.64.0 → v0.65.0 — `BuildUpsert` preconditions become matchable sentinels + a unix-socket PostgreSQL host refuses the TLS it cannot carry
...
- gist: pgx v5 dials a host that is an absolute path over a unix socket and appends no TLS
  ...
  `database.tls` `ConfigError` on every door it guards (C65.6, ADR-062 amendment).
