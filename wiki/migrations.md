...
| E65 | v0.64.0 → v0.65.0 | breaking (C65.1 — a PostgreSQL section with no `connectionstring`, an absolute-path `host` (`/…`, or a drive path such as `C:\pg`) and a `database.tls` block setting `ca`/`cert`/`key` or a `mode` other than `disable` is refused with a `database.tls` `ConfigError` at startup, at the first `deps.DB(ctx)` for a `DBConfigProvider` result, and in `go-bricks-migrate` at its next pin bump, where it used to connect over the unix socket with TLS silently skipped; socket hosts without TLS, TCP hosts, raw `connectionstring` sections and Oracle are unchanged) | 1 | none | grep static config and read every `DBConfigProvider` implementation for an absolute-path `host` beside a `database.tls` block, then remove the block or move to a TCP host (C65.1) |
...
## E65 · v0.64.0 → v0.65.0 — a unix-socket PostgreSQL host refuses the TLS it cannot carry

- gist: pgx v5 dials a host that is an absolute path over a unix socket and appends no TLS
  config there, so a typed PostgreSQL section pairing `host: /var/run/postgresql` with a
  `database.tls` block booted green and connected in plaintext. `validatePostgreSQLFields` now
  judges the host's transport before the material/mode rule and refuses that pairing with a
  `database.tls` `ConfigError` on every door it guards (C65.1, ADR-062 amendment).

---

### [C65.1] a unix-socket PostgreSQL host refuses a `database.tls` block that asks for TLS · breaking · when: match

- detect: the population is every PostgreSQL section with no `connectionstring` whose `host`
  starts with `/`, or with an uppercase drive letter and `:\`, AND whose `database.tls` sets
  `ca`, `cert` or `key`, or a `mode` other than `disable`. Static config:
  `git grep -nE 'host:[[:space:]]*.?(/|[A-Z]:\\)' -- '*.yaml' '*.yml'`, then read the `tls:`
  block of each hit's section; check env overrides (`DATABASE_HOST` and the named-database and
  ...
  path beside a non-zero `TLS` — a host that arrives from a config server, a tenant record or a
...
- scope: `validatePostgreSQLFields` gains a rule between the mode allowlist and the
  material/mode coherence check. When the trimmed `host` matches pgx's `isAbsolutePath`
  ...
  is NOT covered — `host=/var/run/postgresql` inside a DSN is still accepted
  (gaborage/go-bricks#1551). No signature moves.
- gate: match = such a section, override or provider branch exists.
- apply: pick one exit per section. (a) Remove the `database.tls` block — the socket is local
