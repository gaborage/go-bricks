...
```text
v0.39.1 ─E40─ v0.40.0 ─E401─ v0.40.1 ─E41─ v0.41.0 ─E42─ v0.42.0 ─E43─ v0.43.0 ─E44─ v0.44.0 ─E45─ v0.45.0 ─E49─ v0.49.0 ─E50─ v0.50.0 ─E51─ v0.51.0 ─E52─ v0.52.0 ─E55─ v0.55.0 ─E56─ v0.56.0 ─E57─ v0.57.0 ─E58─ v0.58.0 ─E581─ v0.58.1 ─E59─ v0.59.0 ─E60─ v0.60.0 ─E61─ v0.61.0
```
...
- gist: Two of the framework's own 400 details echoed caller-controlled text. `FieldError.Value`
  ...
  renders the value validation judged rather than the caller's padded original (C61.10).
...
- detect: `grep -n 'flyway.url' <your flyway.conf(s)>` — every conf the migrate path uses, including
  ...
  environment-derived rather than typed. Read every `tls.mode` value the same way: once trimmed it
...
- scope: for a PostgreSQL config using discrete fields, the framework now builds
  ...
  connection now that the `DB_SSL*` export is gone). A `database.tls.mode` outside `disable`/`allow`/`prefer`/`require`/`verify-ca`/`verify-full`
...
- apply: delete `flyway.url` from the conf (it is dead weight that reads as authoritative), along
  ...
  reads its JDBC URL from `flyway.conf`. Put `sslmode`/`sslrootcert` on THAT url too, and
  confirm it names the same host and database as the DSN — otherwise the migration runs
...
- verify: point a throwaway tenant's `host` at `h/?sslmode=disable&x=` and confirm the migrate
  ...
  `ErrInvalidMigrationTLSMode` rather than that value landing in the URL's `sslmode`; blank that tenant's `host`
...
- scope: the column, clause and table-name validators judged the TRIMMED identifier and returned
  ...
  trimmed before rendering. One door is deliberately NOT covered: `BuildUpsert`'s conflict,
  insert and update COLUMN keys go through a separate acceptance system that never normalizes,
  so on PostgreSQL a padded key still renders verbatim inside the quoted identifier
  (`"  name  "`), while Oracle trims it because its renderer always did. Its table argument IS
  normalized. That asymmetry is tracked separately.
...
- verify: `qb.Select("name").From(" users ").ToSQL()` renders byte-identically to the same call
  ...
  JOIN table. `BuildUpsert` is the counter-example to check by hand if you use it: a padded
  column KEY renders verbatim on PostgreSQL and trimmed on Oracle.
