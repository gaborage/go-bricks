...
- detect: a PostgreSQL section (or one whose scheme or keyword form infers PostgreSQL) whose
  `connectionstring` names a non-empty `service=`, as a keyword pair or a URI query parameter,
  or names no `service` key at all while `PGSERVICE` is non-empty in the process environment.
...
- scope: `scanPostgresDSN` records the presence and value of the `service` key, and
  `validatePostgreSQLConnectionString` refuses AHEAD of `[C65.2]`'s rules (1) and (2), whatever
  `PGHOST` or a DSN `host=` says, when the effective service is non-empty: the DSN's own
  `service=` when the key is present, otherwise `PGSERVICE`. pgx v5.11.0 merges a service file
