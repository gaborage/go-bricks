...
- detect: nothing in your build flags this. `git grep -n 'ExchangeDeclaration{' -- '*.go'`
  finds every hand-built exchange. Read each one's `Type`. A declaration from
  `DeclareTopicExchange`, `NewTopicExchange`, `DeclareDirectExchange`, `NewDirectExchange`
  or `DeclareQueueWithDLQ` is never in the population.
