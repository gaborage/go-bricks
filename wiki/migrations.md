...
- scope: for a PostgreSQL config using discrete fields, the framework now builds
  ...
  connection now that the `DB_SSL*` export is gone). A FOURTH shape now fails: a config carrying both
  ...
  unchanged: **Oracle** (no `database.tls` exists for it per ADR-062, and no Oracle URL is built),
...
- apply: delete `flyway.url` from the conf (it is dead weight that reads as authoritative), along
  ...
  For a `connectionstring` tenant, move the TLS settings into the DSN itself
  (`?sslmode=verify-full&sslrootcert=/etc/ssl/ca.pem`) and delete the `database.tls` block —
  the DSN is the escape hatch, and it is used verbatim.
