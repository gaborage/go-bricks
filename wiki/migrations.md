...
- gist: The `go-bricks-migrate` CLI now validates every tenant's resolved database config the
  ...
  second time from the context, so the two always carry the same id where they disagreed on
  every publish out of an HTTP-originated context (C60.10).
...
- preflight: if you run `go-bricks-migrate`, `go-bricks-migrate info` per tenant before the bump
  ...
  that property now holds (C60.10)
...
### [C60.10] a published `CorrelationId` matches the message's own `X-Request-ID` · silent-behavior · when: always
...
- apply: nothing in your code. Repoint any consumer-side join, dedup key or saved query that
  ...
  join on the message's own `X-Request-ID` header, which is now both values.
- verify: publish from an HTTP handler serving a request that carried a `traceparent`, and
  capture the message at the broker: `CorrelationId`, the `X-Request-ID` header and the
  `traceparent`'s trace-id are one string. Before this change the property held the request's
  UUID while the header held the trace-id.
