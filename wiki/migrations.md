...
- detect: ask the SERVER, because both names are normally derived from a tenant id at
  ...
  because three of them are byte-identical to consts that run against a real server as their
  oracle: `pgPublicGrantDetectSQL`, `pgReservedRoleDetectSQL` and
  `pgPublicSchemaResidueDetectSQL` in `migration/roles_integration_test.go`. The lone
  ...
  **Deciding the schema half.** Ownership is not the fingerprint, and `\dn+` will not tell you
  ...
  `public`, and BOTH of a tenant's roles carrying one is the strong signal. Both spellings of
  the setting are matched because the helper quotes the identifier (`search_path="public"`)
  where a hand-edited script usually does not. Rows with `setrole = 0` are database-wide
