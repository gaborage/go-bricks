...
- detect: nothing in your build flags this — no signature moves, and the change is a
  ...
  typed consumer reads: a go-bricks outbox relay always stamps, so those queues are out of
  the population entirely, and only a queue fed by a foreign or hand-rolled producer can
  present the new shape. Neither grep reaches a producer outside your repo — that half is a
...
- gate: match — a typed consumer reads `Meta.DedupKey()` on a queue a foreign producer feeds,
  and that producer sets `message_id` without the outbox stamp. no-match — every producer on
  every queue you consume is a go-bricks outbox relay (always stamped, so the property is
  never reached), or you compose the ledger key yourself instead of calling `DedupKey`, or
  your consumers are sealed.
