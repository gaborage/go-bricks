...
- scope: four consumer-visible changes. The first two are in the sensitive-data filter;
  the last two are NOT — the panic summary is `scheduler/module.go` and the WARN is
  `logger/logger.go`, and the scheduler one is the change most likely to be missed
  because nothing about it looks like a log-filter concern.
...
- gate: match on ANY of — (a) you log decoded JSON, request/response bodies, broker payloads
  ...
  a secret inside it — see detect (4).
  no-match = you log only scalars and typed structs, never assert on `FilterValue`'s result,
  your needle lists are literal and clean, you have no scheduler alert on the panic summary,
  and you log no pre-encoded JSON. Note (a) is the common case and its old behaviour was a
...
- apply: nothing to write for (a) — the crash stops. For (b), change the assertion ONLY where the element type is
  ...
  [#1133](https://github.com/gaborage/go-bricks/issues/1133).
