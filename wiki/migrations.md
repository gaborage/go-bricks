...
- detect: nothing in your build flags this — no signature moves, and the change is a
  ...
  then read each one for what it does with the error: a handler that returns it, logs it,
  counts it, or branches on `errors.Is(err, messaging.ErrInvalidEventID)` to DROP, DLQ or
  alert on an unstamped delivery is in the population, because that arm stops firing for a
  delivery that carries a `message_id`. `git grep -nE 'ErrInvalidEventID' -- '*.go'` catches
...
- scope: `Metadata.DedupKey()` gains a second source on the UNSEALED branch only. When the
  ...
  superseded on that point. Additive: `Metadata.MessageID()` exposes the property.
