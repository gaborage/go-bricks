...
- detect: ask the SERVER, because both names are normally derived from a tenant id at
  ...
  -- RuntimeRole half: a reserved role name never became a role (the server
  -- refuses 'public' and 'pg_%'), so look for the grant it produced instead.
  ...
  and the second is the fingerprint of a `RuntimeRole` that was spelled `public`: DML on a
  tenant schema granted to PUBLIC. The role query reads the ACL out of `pg_class` directly —
  `aclexplode` breaks `relacl` into one row per privilege and represents the PUBLIC
  pseudo-role as `grantee = 0`, so the row appears whoever owns the table. Do NOT use
  `information_schema.role_table_grants`: its documented difference from `table_privileges`
  is that it OMITS what a grant to PUBLIC made accessible, so on an affected instance it
  returns nothing and reads as a clean bill of health. `SELECT table_schema, table_name,
  privilege_type FROM information_schema.table_privileges WHERE grantee = 'PUBLIC' AND
  table_schema NOT IN ('pg_catalog', 'information_schema')` is the portable spelling of the
  same question if you need one. Both forms exclude the catalog schemas because PostgreSQL
  grants PUBLIC `SELECT` on its own catalogs by design; it grants nothing to PUBLIC on a
  table you created, so every remaining row is worth investigating — but a row is not proof
  on its own, since a deliberate `GRANT SELECT … TO PUBLIC` on a shared reference table
  looks identical. Corroborate each row against the spec that provisioned that schema.
  ...
  the server refuses outright, so it leaves no table grant behind. The source side is the
...
- apply: (a) fix the names BEFORE the bump; the framework will not do it for you, because
  ...
  so change the spec's field to a real name and undo what the PUBLIC grant handed out with
  `REVOKE ALL ON ALL TABLES IN SCHEMA "tenant_a" FROM PUBLIC` plus the matching `REVOKE ALL
  ON SCHEMA "tenant_a" FROM PUBLIC`. Then rerun provisioning with the new names so both
...
- verify: rerun your provisioning call and check it returns nil; the refusal is
  ...
  moved with `SELECT schemaname, count(*) FROM pg_tables GROUP BY 1`, and that the
  `aclexplode`/`grantee = 0` query above now returns no row for a tenant schema (a row for a
  reference table you share on purpose is expected and stays).
