...
- gist: Two of the framework's own 400 details echoed caller-controlled text. `FieldError.Value`
  ...
  reporting NULL counting as nil — while the ordering doors refuse every one of those forms
  (C61.11).
...
- scope: the JoinFilter value doors built `col op ?` with exactly one placeholder, so a nil
  ...
  two doors cannot disagree about what a list is.
...
- apply: for equality, nothing — the new rendering is what the call always meant; if you worked
  ...
  rows, so the call site needs to decide what it meant.
- verify: `jf.Eq("u.id", nil)` and `jf.Eq("u.id", sql.NullString{})` both render `u.id IS NULL`;
  ...
  `errors.Is(err, dbtypes.ErrOrderingOperandNotComparable)`. Note `jf.Between` reports squirrel's
  own message for a nil bound rather than this sentinel — it never built `col op ?` and is
  untouched here.
- ref: `database/internal/builder/join_filter.go` (`compare`, `isNullOrList`, `resolveOperand`) ·
