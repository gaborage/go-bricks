...
- note: after the AMQP client reconnects, the consumer registry records the new channel generation
  ...
  volume can fire where it stayed quiet. Only the client `messaging.NewAMQPClient` returns
  re-declares: an `AMQPClient` of another type from a custom `app.Options.MessagingClientFactory`
  keeps the old behavior. No configuration key.
