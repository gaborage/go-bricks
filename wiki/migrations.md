...
- detect: ask the SERVER, because both names are normally derived from a tenant id at
  ...
  **Database-wide default ACLs are out of scope on purpose.** A `pg_default_acl` row can
  ...
  schema to attribute it to or to revoke it against. If you want to audit them anyway, run
  the third arm alone with the join and the `nspname` filter removed.
  ...
  **Expected baseline noise**: the built-in `public` schema carries a PUBLIC `USAGE` grant on
  ...
  `public` is exactly the schema a reserved-name spec provisions into. Likewise the catalog
...
- gate: match = the schema detect shows a tenant schema of yours that is `public`,
  ...
  reserved name to either role — corroborated by the grant detect for `RuntimeRole`, and by
  the source read alone for `MigratorRole`, which leaves no server-side trace. no-match = every tenant
