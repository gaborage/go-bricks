...
## E64 · v0.63.0 → v0.64.0 — both `SetMap` doors order their columns by the caller's names and the JoinFilter family spells inequality `<>` + the JOSE error envelope gates its `details` like every other renderer + PostgreSQL refuses `#` in an unquoted identifier
...
- gist: `InsertQueryBuilder.SetMap` sorted the caller's column names and quoted in that
  ...
  move (C64.4, ADR-101). The same helper also read error-ness ALONE, discarding both returned
  keys, so a store handing back a cached key alongside an error passed it: each lookup's
  key is now checked under a nil guard and reported by dynamic TYPE only, since rendering
  an `*rsa.PrivateKey` would print the digits of `D` and `Primes` into the test log
  (C64.6, ADR-102).
...
- scope: every builder door validates each unquoted segment of every identifier argument
  ...
  byte caps stay unjudged at every door (#1437).
