...
| E50  | v0.49.0 → v0.50.0 | config-break | 4 | none | Flyway migrate surfaces unparseable/failure output as an error; non-empty DB passwords < 8 bytes rejected at config validation + migrate; dev CORS wildcard opt-in; composite tenant resolver default order flips to subdomain → path → header |
...
### [C50.4] composite tenant resolver default order flips to subdomain → path → header · silent-behavior · when: no-match

- detect: find every composite resolver across ALL config sources, then check whether each one pins an explicit order. YAML: `git grep -nE "type: *composite" -- '*.yaml' '*.yml'` and look for an `order:` key in the matched `multitenant.resolver` block. Environment (env vars outrank YAML): `MULTITENANT_RESOLVER_TYPE=composite` with no `MULTITENANT_RESOLVER_ORDER` — sweep shell profiles, `.env` files, compose files, and deployment manifests (`git grep -rn "MULTITENANT_RESOLVER_" ; grep -rn "MULTITENANT_RESOLVER_" k8s/ deploy/ .env* 2>/dev/null`). A `composite` resolver with no explicit order from **any** source is a no-match. (The env value is a comma-separated list, e.g. `MULTITENANT_RESOLVER_ORDER=header,subdomain,path`.)
