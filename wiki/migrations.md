...
- gist: `PGRoleSpec.Validate` judged `Schema` by shape alone — the bare-identifier grammar
  and the 63-byte cap — and `public` passes both. A deployment deriving the schema name
  ...
  `ALTER DEFAULT PRIVILEGES` statements building. `Schema` now additionally passes a
  reserved-name rule — `public`, `information_schema`, and anything under the `pg_` prefix,
  matched case-insensitively — refused with the new `ErrReservedPGSchema`, wrapped as every
  other refusal on this boundary is inside `ErrInvalidPGIdentifier` plus the field name and
  value. The rule binds `Schema` only, and it runs after the identifier floor and BEFORE a
  caller's `IdentifierPolicy`, so no policy can waive it (C65.4, ADR-061 amendment).
...
### [C65.4] `PGRoleSpec.Schema` refuses PostgreSQL's reserved schema names · breaking · when: match

- detect: ask the SERVER, because the schema name is normally derived from a tenant id at
  ...
  run on every instance you provision into; `public` and the catalog schemas are present on
  every PostgreSQL database, so the question the result answers is whether any of them is a
  schema YOU provisioned as a tenant's. Cross-check the call sites that build the spec with
  `git grep -n 'PGRoleSpec{' -- '*.go'` and read what each one assigns to `Schema` — a
  constant `"public"` is found here, a `fmt.Sprintf` over a tenant id is not.
- scope: `Validate` refuses a `Schema` whose lowercased form is `public` or
  `information_schema`, or starts with `pg_`, returning the new exported sentinel
  `ErrReservedPGSchema` wrapped inside `ErrInvalidPGIdentifier` and the `Schema="…"` pair, so
  an existing `errors.Is(err, migration.ErrInvalidPGIdentifier)` matcher keeps matching and
  the message still names the field and the value. Both entry points that build role
  statements call `Validate`, so `ProvisionPGRoles` and `PGRoleProvisioningSQL` both refuse.
  Matching is case-insensitive on purpose: this path QUOTES identifiers, so `"Public"` is a
  distinct schema, but anything spelling the name unquoted folds it to the shared one.
  **Unchanged**: `MigratorRole` and `RuntimeRole`, which may carry these spellings; the
  identifier floor, which still runs first, so a name that is both reserved-shaped and
  outside the grammar comes back as a floor refusal; and every signature on the path.
- gate: match = the detect shows a tenant schema you provisioned that is `public`,
  `information_schema`, or `pg_`-prefixed, in any letter case. no-match = every tenant schema
  you provisioned is a name of your own, which is the common case — `public` existing on the
  instance is not itself a match.
- apply: (a) rename the schema BEFORE the bump; the framework will not rename it for you,
  because moving a populated schema is a data operation. `ALTER SCHEMA "public" RENAME TO
  tenant_a` moves the objects in one statement, then rerun provisioning with the new
  `Schema` so both roles' `search_path` and the default privileges follow, then repoint any
  application connection or `flyway.conf` that named the old one. A schema genuinely shared
...
- verify: rerun your provisioning call and check it returns nil; the refusal is
  `errors.Is(err, migration.ErrReservedPGSchema)` if it does not. Then confirm the objects
  moved with `SELECT schemaname, count(*) FROM pg_tables GROUP BY 1`.
