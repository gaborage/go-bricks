...
- detect: `git grep -n "type: *composite" -- '*.yaml' '*.yml'` then check whether the matched `multitenant.resolver` block sets an explicit `order:` key — a `composite` resolver present without `order:` is a no-match
