...
- gist: Two of the framework's own 400 details echoed caller-controlled text. `FieldError.Value`
  ...
  a Valuer holding a value binds the value it resolves to rather than the wrapper (C61.11).
...
- detect: `git grep -nE '(^|[^[:alnum:]_])jf[.](Eq|NotEq|Lt|Lte|Gt|Gte)[(]' -- '*.go'` and read each
  ...
  too: a `driver.Valuer` HOLDING a value keeps `col op ?` and changes its ARGUMENT. Reading each call site
...
- apply: for equality, nothing — the new rendering is what the call always meant; if you worked
  ...
  wrapper, which now reads as the resolved value.
- verify: `jf.Eq("u.id", nil)` and `jf.Eq("u.id", sql.NullString{})` both render `u.id IS NULL`;
  ...
  not just the SQL, or this one is invisible. Note `jf.Between` reports squirrel's
