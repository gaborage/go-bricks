...
### [C61.4] the framework builds the PostgreSQL Flyway JDBC URL · silent-behavior · when: match
...
- scope: for a PostgreSQL config using discrete fields, the framework now builds
  ...
  connection now that the `DB_SSL*` export is gone). There is
  no escape hatch. Three shapes are
  unchanged: **Oracle** (no `database.tls` exists for it per ADR-062, and no Oracle URL is built),
  **`database.connectionstring`** (the framework does not parse DSNs), and a **`database:` block
...
- verify: point a throwaway tenant's `host` at `h/?sslmode=disable&x=` and confirm the migrate
  ...
  without `tls.mode`. Then run a migrate with `--verbose` (or read the process list) and confirm the argv carries
