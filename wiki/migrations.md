...
- detect: `git grep -n 'DeclareQueueWithDLQ(' -- '*.go'` over your own modules. EVERY call
  ...
  management API) for each name the greps produce and each `<name>.dlq` beside it. A queue the
...
- gate: match = the broker already holds a classic `<queue>.dlq`, or a
  classic PRIMARY queue, declared by this helper. RabbitMQ's declare-equivalence check refuses
