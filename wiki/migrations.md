...
- preflight: **nine** actions. (i) If a proxy in front of the service sits on
  ...
  **Oracle** call whose conflict column and insert key name the same column in
  ...
  one before the bump. Four shapes now abort startup: a PostgreSQL `mode`
  ...
  as "no TLS configured" but pgx defaults to `prefer`, so a `ca:` sitting
  alone was never verifying anything. Remediating the second shape is not
  ...
  here is compiler-caught; booting each environment decides.
...
- verify: `go build ./...`, then start the app against the database in each environment — a rejected
  shape fails startup with `database.tls` (or `database.tls.mode`) named in the error and the remedy
  in its action line.
