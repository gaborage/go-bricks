...
| E581 | v0.58.0 → v0.58.1 | silent-behavior | 2 | none | if you cache any type carrying a time.Time, decide before the bump whether a compare-and-set on a sub-second timestamp may fail during the rolling deploy (C581.1); and if you set `observability.logs.samplingrate` to any value strictly between 0.0 and 1.0, expect the exported INFO/DEBUG log volume to change — a sub-1% rate exported nothing before the bump and starts exporting after it (C581.2) |
...
- gist: The cache's CBOR encoder moves from `cbor.TimeRFC3339` to
  ...
  0.01%-resolution one (`%10000`), so a sub-1% rate that used to truncate to
  a zero threshold and export nothing now exports its configured fraction.
- build-caught: none
- preflight: if any cached type carries a `time.Time` and you use
  ...
  (`ttl == 0`) entries keep the old encoding until overwritten; and if you
  set `observability.logs.samplingrate` to any value strictly between 0.0
  and 1.0, expect the exported INFO/DEBUG log volume to change — a sub-1%
  rate exported nothing before the bump and starts exporting after it
...
- scope: one constant in `observability/dual_processor.go`. The sampling comparison scaled from `%100 < uint64(rate*100)` to `%10000 < round(rate*10000)`. Config validation is unchanged — `[0.0, 1.0]` is still the accepted range, and no previously-valid value is rejected.
...
- after: three consequences. (i) Any rate below 0.01 that is not zero used to truncate to a zero threshold and export **nothing**; it now exports the configured fraction — `0.005` goes from 0% to 0.5% of INFO/DEBUG trace logs, a real increase in export volume and cost. (ii) Rates that are not whole percents stop flooring — `0.999` was 99% and is now 99.9%, `0.155` was 15% and is now 15.5%. (iii) The **membership** of the sample changes for every fractional rate, including whole-percent ones: the same expected fraction is kept, but the modulus moved from 100 to 10 000, so which traces land in the sample is redrawn. Sampling stays deterministic per trace after the bump; it is simply not the same set as before. Nothing to do for (iii) — but do not read a trace disappearing from the sample as a regression.
