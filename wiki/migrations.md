...
- gate: match = any log-backend query filtering, at record level, either one of the four framework identity keys **or** any key your deployment sets via `OTEL_RESOURCE_ATTRIBUTES`/`OTEL_SERVICE_NAME`. no-match = you have confirmed both — an environment with `OTEL_RESOURCE_ATTRIBUTES` set is not cleared by checking the four framework keys alone
- before:
