...
## E63 · v0.62.0 → v0.63.0 — the typed publish door replaces raw byte publishing + header-sourced event ids are validated before the inbox ledger + the query builder grows four clauses
...
## E64 · v0.63.0 → v0.64.0 — `jose/sealed` gains a raw-document door and enforces the Subject case-fold rule on the serialized bytes

- gist: `jose/sealed.Seal` was the only sealer, and it takes a Go value of the scanned
  `Spec.Type`. A tool or a JSON-fixture test has bytes and no Go type, so neither could
  ...
  (C64.1, ADR-097 amendment). Reaching that door exposed a hole in the typed one: the G9
  ...
  the rule to the serialized document, which refuses at publish time what v0.63.0 signed
  (C64.2). The opener is untouched: its rule table, codes and rule numbering are unchanged,
  so every message v0.63.0 produced still opens. The new door also gets its first consumer:
  `cmd/seal-event` seals one JSON object from the shell, so a queue can be fed a real sealed
  message without a Go program (C64.3).

---

### [C64.1] `jose/sealed` gains `NewDocumentSpec` + `SealDocument` — sealing an already-serialized document · additive-optional
...
### [C64.2] the Subject case-fold twin check runs on the serialized bytes for the typed door · breaking · when: match
...
### [C64.3] seal-event CLI · additive-optional

- note: New installable `cmd/seal-event` CLI (`go install
  github.com/gaborage/go-bricks/cmd/seal-event@v0.64.0`) seals one JSON object as a sealed
