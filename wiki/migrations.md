...
```text
v0.39.1 ─E40─ v0.40.0 ─E401─ v0.40.1 ─E41─ v0.41.0 ─E42─ v0.42.0 ─E43─ v0.43.0 ─E44─ v0.44.0 ─E45─ v0.45.0 ─E49─ v0.49.0 ─E50─ v0.50.0 ─E51─ v0.51.0 ─E52─ v0.52.0 ─E55─ v0.55.0 ─E56─ v0.56.0 ─E57─ v0.57.0 ─E58─ v0.58.0 ─E581─ v0.58.1 ─E59─ v0.59.0 ─E60─ v0.60.0 ─E61─ v0.61.0 ─E62─ v0.62.0 ─E63─ v0.63.0 ─E64─ v0.64.0 ─E65─ v0.65.0 ─E66─ v0.66.0 ─E67─ v0.67.0
```
...
### [C62.2] an absent `cache.critical` leaves the cache probe non-critical; `CacheConfig.Critical` is a plain `bool` · silent-behavior + compile-break · when: match
...
- scope: `Config.IsCacheCritical` answers `false` for an unset `Critical` and for a nil
  ...
  fails resolution (ADR-077), and `multitenant.tenants.<id>.cache.critical` still parses and is
  still ignored.
- gate: match = EITHER `cache.critical` is absent in a cache-enabled deployment
  ...
  explicitly through YAML or env, `true` or `false`, and nothing watches the WARN line — nothing
  changes for you.
- apply: set `cache.critical: true` (or `CACHE_CRITICAL=true`) in that deployment BEFORE the
  ...
  alert on `cache.critical is explicitly false`, and the key itself may stay or go. In Go,
  `Critical: new(true)` becomes `Critical: true`, `Critical: new(false)` becomes `Critical: false`
  or is deleted, and a `*cfg.Cache.Critical` / `== nil` read becomes `cfg.IsCacheCritical()`.
- verify: with the cache enabled and Redis stopped,
  ...
  body carries `"cache":"unhealthy"`) and `503` once `cache.critical: true` is set; `go vet ./...`
  is clean with no `*bool` left on the field.
...
## E63 · v0.62.0 → v0.63.0 — the typed publish door replaces raw byte publishing + header-sourced event ids are validated before the inbox ledger + the query builder grows four clauses + `jose/sealed` grows a raw-document door + the seal-event CLI

- gist: `Declarations.DeclarePublisher` registered a destination and handed back nothing;
  the bytes went through `AMQPClient.PublishToExchange(ctx, PublishOptions, []byte)` or the
  default-exchange `Client.Publish`, so every module re-spelled exchange and routing key at
  ...
  `outbox.EventIDFromHeaders` accepted any non-empty `x-outbox-event-id`. The sealed typed
  door (ADR-097, landing separately) keys the same ledger on `<SignFamily>:<jti>`, and `jti`
  is cleartext in a captured message's protected header — so a publish-ACL holder on an
  unsealed sibling consumer's queue in the same service could pre-insert the sealed message's
  key through a rewritable header and have the legitimate sealed event skip+ACK silently. The
  ledger door now refuses every id outside `^[A-Za-z0-9_-]{1,128}$`; `:` is outside it, so no
  header-sourced id can spell a sealed key. `messaging.Metadata` gains `DedupKey()` (the
  grammar-validated header, or the typed error) and `Sealed()` (always `false` for a plain
  typed consumer — per type, never per message), and the silent `!inserted` short-circuit —
  the only observable of a replay campaign — gains a counter and a log line (C63.2).
  Separately on the database side: the ledger-store migration (#1255) found four recurring
  SQL shapes the builder could not carry, and the builder grows them — a row lock, an
  argument-carrying SET expression, scalar subqueries in a projection, Oracle's `FROM dual`
  — as methods on the exported builder interfaces, which is a compile break for any
  consumer type that implements them (C63.3, ADR-098).
  Separately on the sealing side: `jose/sealed.Seal` was the only sealer, and it
  takes a Go value of the scanned `Spec.Type`. A tool or a JSON-fixture test has
  bytes and no Go type, so neither could
  produce a sealed message at all. `NewDocumentSpec` + `SealDocument` add the door: the same
  envelope, the same `jti`/`iat`/`sp`/`etyp`/`tid` slots, and the caller's own bytes signed
  verbatim except the Subject member's value, so a hand-written fixture is reproducible
  (C63.4, ADR-097 amendment). Reaching that door exposed a hole in the typed one: the G9
  case-fold rule — no clear member may case-fold to the Subject, because `encoding/json`
  matches members case-insensitively on decode and a consumer would read the clear twin —
  lived only in the struct scan, which sees declared fields. A type with a custom
  `MarshalJSON` could emit `Card` beside the sealed `card` and seal it. Both doors now apply
  the rule to the serialized document, which refuses at publish time what v0.62.0 signed
  (C63.5). The opener is untouched: its rule table, codes and rule numbering are unchanged,
  so every message v0.62.0 produced still opens. The new door also gets its first consumer:
  `cmd/seal-event` seals one JSON object from the shell, so a queue can be fed a real sealed
  message without a Go program (C63.6).
...
- detect: `git grep -nE '[.]PublishToExchange\(|[.]Publish\(|messaging[.]PublishOptions\{|messaging[.]ValidatePublishDestination\(' -- '*.go'`
  ...
  `JobContext.Messaging()` return), `PublishOptions` is no longer exported, and
  `ValidatePublishDestination` takes `(exchange, routingKey string, headers map[string]any)`.
  Hits on the second line are a `scheduler.JobContext` implementation of your own whose
  `Messaging()` must now return `messaging.AMQPClient`. Hits on the third are compile
  breaks in your tests: the byte doubles are removed with the interface methods. A hit on
...
- scope: `DeclareTypedPublisher[T](decls, opts) *Publisher[T]` is the only module-facing
  ...
  pre-flight and the ADR-087 tenant stamp are unchanged behind the handle. Removed from the
  exported surface: `Client.Publish`, `AMQPClient.PublishToExchange`, the `PublishOptions`
  type, `AMQPClientImpl.Publish`/`.PublishToExchange`, `testing/mocks.MockMessagingClient.Publish`
  with `ExpectPublish`/`ExpectPublishAny`, `testing/mocks.MockAMQPClient.PublishToExchange` with
  `ExpectPublishToExchange`/`ExpectPublishToExchangeAny`/`PublishedFrames`/`LastPublishedFrame`/
  `ClearPublishedFrames`, and the `PublishedFrame` type. Changed: `ValidatePublishDestination(exchange, routingKey string, headers map[string]any) error`
  (was `(PublishOptions)`); `scheduler.JobContext.Messaging()` returns `messaging.AMQPClient`
  (was `messaging.Client`), the same type as `ModuleDeps.Messaging`. Added: `messaging.EventPublisher[T]`
  (the seam `*Publisher[T]` satisfies), `messaging/testing.CapturePublisher[T]` (records
  `T` values; `NewCapturePublisher`, `Publish`, `Events`, `Last`, `Fail`, `Reset`), and
  `messaging.ErrPublishDoorUnavailable` — returned by `Publisher[T].Publish` for a client
  that carries no framework byte door (a hand-written `AMQPClient`, an
  `app.Options.MessagingClientFactory` product, a `testing/mocks` double). Framework
  internals (outbox relay, stamping wrapper, manager lease) keep a bytes door on an
  unexported type you cannot reach; the relay's seam is `internal/publishdoor`.
- gate: match = you call `Publish`/`PublishToExchange` on a client obtained from
  `deps.Messaging(ctx)` or `JobContext.Messaging()`, name `messaging.PublishOptions`, call
  `ValidatePublishDestination`, implement `scheduler.JobContext`, or inject the removed
  mock methods. no-match = you publish only through the outbox (`deps.Outbox.Publish` with
  a struct payload), through a `Publisher[T]` handle already, or the streams lane — nothing
  to do.
- apply: In `DeclareMessaging`, replace `decls.DeclarePublisher(&messaging.PublisherOptions{…}, ex)`
  ...
  `RoutingKey`. `ValidatePublishDestination(messaging.PublishOptions{Exchange: e, RoutingKey: r, Headers: h})`
  becomes `ValidatePublishDestination(e, r, h)`. A `JobContext` implementation changes its
  `Messaging()` return type to `messaging.AMQPClient`. Tests: store the handle behind
  `messaging.EventPublisher[T]` and inject `messagingtesting.NewCapturePublisher[T]()`,
  asserting `Events()`/`Last()` on typed values instead of the removed mock methods; a
...
- detect: `git grep -nE 'ProcessOnce\(|EventIDFromHeaders\(|x-outbox-event-id|HeaderEventID' -- '*.go'`
  ...
  hand-set header, an upstream bridge, a partner producer, or a consumer that BUILDS its
  own `ProcessOnce` key (a business key, `<type>:<ref>`, an email, a URL path) may not — a
  repo grep cannot see another service's producer, so check it before taking the no-match path.
- scope: `ProcessOnce` runs `messaging.ValidateEventID` before opening the transaction and
  returns an error wrapping `messaging.ErrInvalidEventID` for an empty id, one over 128
  bytes, or one carrying any byte outside `[A-Za-z0-9_-]` — no row is written. Returned
  from a handler, that error is nacked without requeue like any other poison message, so
  the delivery parks on the queue's DLQ (pair the queue with `DeclareQueueWithDLQ`) where
  v0.62.0 inserted the row and ran the handler. The message names the byte LENGTH, never
  the id. `outbox.EventIDFromHeaders` is unchanged (extraction only) and `outbox.HeaderEventID`
  now aliases `messaging.HeaderEventID`. Additive: `messaging.Metadata.DedupKey() (string, error)`
  returns the validated header or the same typed error for an absent, empty or malformed one;
  `messaging.Metadata.Sealed() (messaging.SealedEnvelope, bool)` returns `(zero, false)` for
  every delivery a plain typed consumer receives. The `!inserted` short-circuit records
  `inbox.dedup.hits` (labels `inbox.tenant.present` bool, `inbox.sealed` = `false`) under the
  `go-bricks/inbox` meter and writes one INFO line (`tenantPresent`, `eventIdLength`,
  `sealed` — presence and length only); `inbox.retentionperiod` is documented as the replay
  window, bounding DLQ drains and outbox re-drives as well as broker redelivery.
- gate: match = any producer of the events you consume mints `x-outbox-event-id` outside the
  grammar, or a consumer hands `ProcessOnce` an id it built, or your tests inject such ids.
  no-match = ids are framework-minted UUIDs — nothing to do beyond wiring the counter.
- apply: For every id that can fail the grammar, re-mint it — a UUID, ULID or KSUID per
  event, minted once at the producer and carried in `x-outbox-event-id` — or move the
  producer to the sealed typed door once it ships; do NOT normalize or hash a
  non-conforming id at the consumer, which re-opens the collision the grammar closes.
  In a metadata-aware typed consumer replace the `outbox.EventIDFromHeaders` + `!ok` pair
  with `id, err := meta.DedupKey(); if err != nil { return err }`. Point a dashboard at
...
- amendment (2026-09-03, #1359, additive): `inbox.ProcessOnce` now runs `messaging.ValidateDedupKey(ctx, id)`
...
### [C63.3] the query builder grows `ForUpdate`/`ForUpdateNoWait`/`SubqueryColumn`/`SetExpr` — implementers of the builder interfaces must add them · compile-break · when: match

- detect: `git grep -nE '(dbtypes|types)[.](SelectQueryBuilder|UpdateQueryBuilder)' -- '*.go'`
  Classify each hit. A hit on a type that IMPLEMENTS the interface — a method set
  returning it (`func (m *myDouble) Limit(uint64) types.SelectQueryBuilder`), a `var _
  types.SelectQueryBuilder = (*myDouble)(nil)` assertion, a decorator embedding it — is the
  compile break. A hit that only names the interface as a parameter, return or field type
  (you CALL the builder) needs nothing. `go build ./...` enumerates the first kind.
- scope: `types.SelectQueryBuilder` gains `ForUpdate() SelectQueryBuilder`,
  `ForUpdateNoWait() SelectQueryBuilder` and `SubqueryColumn(sub SelectQueryBuilder, alias
  string) SelectQueryBuilder`; `types.UpdateQueryBuilder` gains `SetExpr(column string, expr
  RawExpression, args ...any) UpdateQueryBuilder`. Framework doubles (`testing/mocks`) already
  carry them. Behavior additions on the framework builder: a row lock renders after pagination
  and is refused on Oracle beside `Limit`/`Offset` (`types.ErrRowLockWithPagination`) and refused
  as a subquery; `SetExpr` is a raw-SQL door (annotate every call site); a table-less Oracle
  `SELECT` now renders `FROM dual` (the previous text was invalid on Oracle).
- gate: match = your code declares a type satisfying either interface. no-match = you only
  call `qb.Select`/`qb.Update` — nothing to do.
- apply: Add the methods to your implementation, delegating or returning the receiver as your
  double does for `Limit`; for a decorator, forward to the wrapped builder. Then adopt the
...
### [C63.4] `jose/sealed` gains `NewDocumentSpec` + `SealDocument` — sealing an already-serialized document · additive-optional

- note: New exported pair on `jose/sealed`:
  `NewDocumentSpec(signLogical, encryptLogical, subjectPath string) (*Spec, error)` builds a
  `Spec` with a nil `Type` (both kids go through `CheckLogicalKid`, an empty path is
  `SEAL_TAG_SUBJECT_MISSING`), and
  `SealDocument(doc []byte, spec *Spec, opts *Options) ([]byte, error)` seals bytes the
  caller serialized, accepting a document `Spec` or a scanned one. The signed payload is
  `doc` byte for byte with only the Subject member's value replaced by the compact JWE, so
  member order, whitespace and number formatting survive. `Options.Validate` no longer
  requires `spec.Type` — requiring a scanned `Spec` is now `Seal`'s own rule, checked before
  `Validate` runs and reported with the same `SEAL_OPTIONS_INVALID` and the same message, so
  a declaration-time caller such as `messaging/sealed` sees no change. `Open` still requires a
  scanned `Spec` (rule 0, `SEAL_OPTIONS_INVALID`): a document `Spec` seals and never opens.
  The typed door remains the production path; this one exists for tooling and fixtures.
- ref: gaborage/go-bricks#1408 · [ADR-097](adr_097_sealed_amqp_messages.md) (amendment) · `jose/sealed/document.go`

### [C63.5] the Subject case-fold twin check runs on the serialized bytes for the typed door · breaking · when: match

- detect: `git grep -ln 'MarshalJSON' -- '*.go'`, cross-checked against the types carrying a
  seal declaration (`git grep -ln 'seal:"subject"' -- '*.go'`). A hit in BOTH lists for the
  same type is the whole population. The Subject's own type marshaling itself is irrelevant —
  only a `MarshalJSON` on the EVENT type (or on an embedded type whose method is promoted to
  it) chooses the top-level member names. A repo grep is sufficient here: the event type is
  yours by definition, since only your code can carry the seal tag.
- scope: `jose/sealed.Seal` and the new `SealDocument` both refuse a document carrying a
  top-level member whose name `strings.EqualFold`s the Subject member's name without being
  equal to it, with `SEAL_DOCUMENT_INVALID` (sentinel `ErrSealFailed`) — the same code the
  door already returns for an absent or duplicated Subject. The error names the declared
  subject path only, never the twin's name or any document byte. Reached through
  `messaging/sealed`, the failure surfaces at publish, so the event is not published; the
  typed publish call returns the error. `ScanType` is unchanged and still refuses a declared
  namesake field at startup. `Open` is untouched — a message sealed before the bump opens
  exactly as before, twin and all.
- gate: match = a seal-tagged event type has its own `MarshalJSON` (or promotes one) that
  emits a member whose name differs from the Subject member's only by case. no-match = every
  seal-tagged type is marshaled by `encoding/json` from its declared fields — `ScanType`
  already enforced the rule at startup, so nothing changes.
- apply: Rename the clear member so it no longer case-folds to the Subject, or drop it from
  the marshal output. Do NOT rename the Subject member instead: its name is the signed `sp`
  manifest entry, so consumers pin it. If the twin was carrying a redacted or masked echo of
  the Subject, give it a distinct name (`cardMasked`, not `Card`).
- verify: `go test ./...`  # then seal one event of each affected type and confirm it still
  publishes; a refused one fails with `SEAL_DOCUMENT_INVALID` naming the subject path
- ref: gaborage/go-bricks#1408 · [ADR-097](adr_097_sealed_amqp_messages.md) (amendment) · `jose/sealed/seal.go`, `jose/sealed/splice.go`

### [C63.6] seal-event CLI · additive-optional

- note: New installable `cmd/seal-event` CLI (`go install
  github.com/gaborage/go-bricks/cmd/seal-event@v0.63.0`) seals one JSON object as a sealed
  AMQP event body via the production `sealed.SealDocument` path, for feeding a queue with
  `rabbitmqadmin publish`. The concrete `-sign-kid`/`-encrypt-kid` generations are split into
  their Logical families for the document `Spec`, so a body it emits is one the sealed
  consume door opens by construction. No config keys, no framework API change — `package
  main` contributes no importable symbols and is inert unless separately installed and
  invoked.
- ref: #1408 · cmd/seal-event/main.go · wiki/sealing.md#minting-test-events-with-rabbitmqadmin-seal-event-cli

---

## E64 · v0.63.0 → v0.64.0 — both `SetMap` doors order their columns by the caller's names and the JoinFilter family spells inequality `<>` + the JOSE error envelope gates its `details` like every other renderer + PostgreSQL refuses `#` in an unquoted identifier and every door judges the vendor's identifier byte cap per segment

- gist: `InsertQueryBuilder.SetMap` sorted the caller's column names and quoted in that
  order; `UpdateQueryBuilder.SetMap` quoted first and sorted the QUOTED spellings. The two
  coincide on PostgreSQL, which renders a column verbatim, and diverge on Oracle, where a
  reserved word gains a leading double quote that sorts ahead of every bare name — so the
  same map rendered `id, "level", "size"` at one door and `"level", "size", id` at the
  other, with the bind positions following. One shared helper,
  `quotedColumnsInNameOrder`, now sorts by the caller's names, validates each key under the
  door's own error context and quotes in that order for both doors; PostgreSQL output is
  byte-identical to v0.63.0. In the same change the last spelling difference between the
  filter families goes: `opNotEqual` is `<>` and `NotEqColumn` renders through the constant,
  so `jf.NotEq`, `jff.NotEqColumn` and `f.NotEq` agree — nil and list operands were already
  aligned by `[C61.11]` (C64.1, ADR-099). Separately, the JOSE post-trust error envelope
  copied `IAPIError.Details()` to the wire ungated while the enveloped and raw renderers
  required `app.debug` AND a development environment; ADR-084 had recorded that as an
  accepted exception because the body is encrypted to an authenticated peer. It is not one —
  the peer decrypts and routinely logs the body — so `buildErrorEnvelope` now renders
  `details` through the same `devDetails` funnel and a production JOSE error carries `code`,
  `message` and `meta` only (C64.2, ADR-084 amendment). Third, the shipped helper
  `keystore/testing.AssertKeyNotFound` used `assert.Error` for its public-key check, so an
  unexpectedly FOUND key was recorded and the helper went on to judge the private key of a
  keystore already known to be in the wrong state — noise in a consumer's own test output.
  That check is now `require.Error` and stops the caller's test at that line; the private-key
  check stays `assert.Error`, being the last statement, and the exported signature does not
  move (C64.4, ADR-101). Finally the byte-cap half ADR-100 had deferred lands: the renderer
  gained `MaxBytes()` — 63 on PostgreSQL, 128 on Oracle, 63 for an unknown vendor — and one
  builder funnel judges every identifier SEGMENT against it before the charset check, quoted
  interiors included, since quoting escapes the alphabet and not the length. The cap is never
  applied to the rendered whole, so a qualified name whose segments each fit still builds; what
  changes is that an over-long name is now a deferred `ToSQL()` error naming the argument, the
  limit and the vendor, where PostgreSQL used to truncate it at 63 bytes and let two prefix-
  sharing names collapse onto one object. Outside the builder, the inbox store's table-name
  bound follows the store's vendor (PostgreSQL 49, Oracle still 114, each the cap minus
  `len("idx__processed")`) while `sqlid.ValidateTableName` stays vendor-blind at Oracle's 128,
  its config-time callers having no vendor in scope (C64.5, ADR-100 amendment). The same helper
  also read error-ness ALONE, discarding both returned keys, so a store handing back a cached
  key alongside an error passed it: each lookup's key is now checked under a nil guard and
  reported by dynamic TYPE only, since rendering an `*rsa.PrivateKey` would print the digits of
  `D` and `Primes` into the test log (C64.6, ADR-102).
  Last on this hop, the connect seam gained its first identity check: a PostgreSQL section with
  no `connectionstring` and an empty-after-trimming `host` is refused with the startup path's own
  `MissingFieldError`, because pgx v5.11.0 replaces an empty host with libpq's default unix
  socket and skips TLS there, discarding the configured material (C64.8, ADR-050 amendment) —
  and the same pgx bump brought libpq's raw-DSN escaping rules with it for anyone passing a
  `connectionstring` verbatim (C64.9). And one compile break lands on this hop: `jose.Policy`
  gains the `Mode`, `Typ`, `ProtectedHeaders` and `IATMillis` fields that express Visa Message
  Level Encryption's encrypt-only shape, and the map among them makes the struct
  non-comparable — `==` and map-key use on a policy stop compiling, exactly as they did for the
  three messaging structs in `[C59.2]`. Every existing policy is otherwise untouched:
  `SealModeJWEofJWS` is the zero value, the nested path's content-encryption floor stays
  `A256GCM`, and `A128GCM` is admitted only behind `SealModeBareJWE` (C64.15, ADR-107).

---

### [C64.1] map-shaped column doors order by the caller's names, and JoinFilter inequality is `<>` · breaking · when: match

- detect: `git grep -nE '\.SetMap\(' -- '*.go'` for the ordering half — only a call whose
  map has TWO OR MORE keys AND runs against Oracle AND includes a key the framework quotes
...
- scope: `UpdateQueryBuilder.SetMap` and `InsertQueryBuilder.SetMap` both render their
  ...
  "size" = :3`) rather than leading the statement, and the argument slice follows the same
  order. Error text, the validation funnel and the padded-duplicate-key behaviour are
  unchanged, and PostgreSQL output is identical to v0.63.0. `jf.NotEq(column, scalar)`
  renders `column <> ?` and `jff.NotEqColumn(l, r)` renders `l <> r`; a nil operand still
  renders `IS NOT NULL` and a slice still expands to `NOT IN`. `BuildUpsert`'s column order,
  `Set`, `SetStruct`, `InsertStruct` and `Columns`/`Values` are untouched.
- gate: (a) SET order — match = a golden SQL string, a query-text matcher, a driver mock
  expectation or a stored plan pins the rendered form of a multi-column `SetMap` statement
  built against Oracle whose keys include a quoted identifier. no-match = every `SetMap` map
  is single-key, or all keys are bare names, or nothing asserts on rendered SQL text.
  (b) `<>` — match = anything compares against SQL text produced by `jf.NotEq` or
  `jff.NotEqColumn` (a golden file, an `assert.Contains` on `ToSQL()`, a logged-statement
  matcher, a database proxy rule). no-match = your assertions read the ARGS and the row
  effect only, never the operator token.
- apply: (a) Re-read each affected expectation against the new order — columns by bare name,
  arguments in that same order — and update the SQL string AND its argument slice together;
  they move as one. Do not re-generate a golden blindly: the argument order is the half a
...
### [C64.2] the JOSE post-trust error envelope gates `error.details` behind debug + a development environment · breaking · when: match

- detect: the population is your PEER, not this codebase — the framework side compiles
  unchanged. On the framework side, `git grep -nE 'WithDetails\(' -- '*.go'` lists the
  handlers that set details at all; cross-reference them with the routes registered under a
  jose policy (`git grep -nE 'JOSE|jose\.Policy' -- '*.go'`), since only those handlers'
  details ever reached a JOSE envelope. On the peer side, grep the client that consumes your
  JOSE routes for a reader of the field — `git grep -nE '"details"|[.]details' -- <peer client code>`
  — and include log pipelines and alert rules that key off `error.details`, which no compiler
  and no Go grep will find for you. Anchor on the QUOTED key or the dotted access, never on a
  bare `details`: `git grep -E` silently ignores `\b`, so a `\bdetails\b` pattern matches
  nothing at all and reads as a clean "no reader" when the reader is right there.
- scope: `buildErrorEnvelope` renders `error.details` through `devDetails`, the funnel the
  enveloped and raw renderers already used: present only when `app.debug` is true AND
  `app.env` is a development alias, and carrying the injected `stackTrace` when the error
  captured one. In every other posture the key is absent from the decrypted body. `code`,
  `message`, `meta` and the HTTP status are unchanged, the envelope is still encrypted with
  the route's outbound policy, and the pre-trust plaintext `{code,message}` envelope is
  untouched — `joseAPIError.Details()` is nil by construction, so it never had a key to gate.
  `classifyError`'s own attach-side gate is unchanged and still bounds what reaches
  `details` in the first place.
- gate: (a) match — a production peer, log pipeline or alert rule reads `error.details` (or a
  member of it) off a JOSE route's decrypted error body. (b) match — a peer reads it only
  against a development deployment running with `app.debug` on. (c) no-match — nothing on
  either side reads `details` from a JOSE error body; the peer reads `code`/`message` only.
- apply: (a) Move the peer off the field before the bump. Anything the peer genuinely needs
  in production belongs in `code` (a stable, documented error code it can branch on) or in
  `message`; re-raise the requirement rather than re-widening the envelope, because the
  disclosure is unbounded — a handler's `WithDetails` and the framework's bind and validation
  diagnostics both land there, and a captured `stackTrace` discloses server file paths into
  the peer's log estate. Delete the alert rules and log-pipeline extractors in the same
  change, or they go silently empty. (b) Nothing to change: that posture still discloses
  details, byte-identical to the standard channel. Confirm the deployment is genuinely
  development-only — the same peer code pointed at production stops seeing the field.
  (c) Nothing to do.
- verify: `go build ./... && go test ./...`  # then (a) drive one failing request over a JOSE
  route against a production-posture deployment (`app.debug` off, or `app.env` not a
  development alias), decrypt the response and confirm the error object has `code`, `message`
  and no `details`, and that the peer handles its absence without erroring; (b) drive the
  same request against the development deployment and confirm `details` is still present and
  matches what the unencrypted channel returns for the same error; (c) no check needed
- ref: gaborage/go-bricks#1163 · [ADR-084](adr_084_response_error_details_carry_no_request_input.md) (amendment) · `server/jose.go`, `server/handler.go`
...
- detect: `git grep -nE '[`"][^`"]*#' -- '*.go'` over the code that CALLS the builder, narrowed
  ...
  nothing. A struct `db:"a#b"` tag also matches this grep and IS covered: the struct doors
  judge tag names against the vendor's alphabet too — see scope.
...
- scope: every builder door validates each unquoted segment of every identifier argument
  ...
  are `Select` (column and `RawExpression` alias), `SubqueryColumn`'s alias, `From`/JOIN (table,
  inline alias, `Table().As()` name and alias), `OrderBy`, `GroupBy`, every Filter/JoinFilter
  column, `InsertWithColumns`, `Insert().Columns`/`SetMap`, `Update`/`Set`/`SetMap`, `Delete`, and
  `BuildUpsert`'s table and column keys. A refusal is a deferred `ToSQL()` error naming the
  argument, the vendor and the offending segment — never a panic. Quoted segments (`"a#b"`,
  `u."a#b"`) and the `*`/`t.*` wildcard are skipped. An unknown vendor gets PostgreSQL's grammar.
  The three struct doors — `InsertStruct`, `InsertFields` and `UpdateQueryBuilder.SetStruct` —
  judge db-tag column names through the same column funnel, so a `db:"a#b"` field is a deferred
  `ToSQL()` error on PostgreSQL and renders on Oracle. The `columns` package still parses tags
  against the union alphabet; the vendor's alphabet is judged at the door (#1449). Identifier
  byte caps are judged at the same doors by the same funnel — see [C64.5].
- gate: (a) identifier arguments — match = any PostgreSQL (or unknown-vendor) deployment hands
  ...
  those statements.
- apply: (a) Rename the column or object so it needs no `#`, or quote the identifier at the call
  site (`"a#b"`) — quoting is exempt on both vendors and is the framework's own reserved-word
  form, but note the server then treats the name as case-sensitive, so `"a#b"` and `a#b` are not
  interchangeable once quoted. A computed expression belongs in `qb.Expr()` as before. (b) Update
  those assertions to expect the `ToSQL()` error on PostgreSQL, and keep the accepting row for
...
- ref: gaborage/go-bricks#1202, gaborage/go-bricks#1449 · [ADR-100](adr_100_vendor_identifier_grammar_behind_renderer.md) · follow-up gaborage/go-bricks#1437 · `database/identifier/identifier.go`, `database/internal/builder/renderer.go`, `database/internal/builder/identifiers.go`

---

### [C64.4] `keystore/testing.AssertKeyNotFound` aborts on an unexpectedly found public key · breaking · when: match

- detect: `git grep -nE 'AssertKeyNotFound\(' -- '*_test.go'` in YOUR tree — the helper ships
  from `keystore/testing`, so the population is your own tests, not this codebase. The open
  paren anchors on a CALL: a bare name also matches a helper you wrapped, a comment naming
  it, or a table of assertion names. Nothing in a build fails here, so a compiler will not
  find these for you.
- scope: the public-key check inside the helper becomes `require.Error`, so a public key that
  is unexpectedly PRESENT calls `t.FailNow()` and the caller's test stops at that line. The
  private-key check keeps `assert.Error` — it is the last statement and invalidates nothing.
  The exported signature is unchanged, and the passing path is untouched: when the key is
  genuinely absent both lookups still return errors and both assertions still pass. This is
  not the end state for this helper on this hop: `[C64.6]` (#1457) also makes each lookup
  check its returned KEY, so read the two together and take the end state from `[C64.6]`.
- gate: (a) match — a call site has statements AFTER the helper that must run even when the
  public key is found (a cleanup, a second assertion, a counter the test reads later).
...
- verify: `go test ./...` in your tree  # then (a) confirm the moved cleanup still runs by
  making one such test fail deliberately (seed the public key it expects to be absent) and
  checking the cleanup's effect is observable; (b) and (c) a green run is the check
- ref: gaborage/go-bricks#1092 · [ADR-101](adr_101_test_helpers_abort_on_first_failure.md) ·
  `keystore/testing/assertions.go`. [C64.3] arrives with #1202's stack, which merges before
  this atom.

---

### [C64.5] every builder door judges the vendor's identifier byte cap per segment, and the inbox table bound follows the store's vendor · breaking · when: match

- detect: ``git grep -nE '[`"][A-Za-z_][A-Za-z0-9_$#]{63,}' -- '*.go'`` over the code that CALLS
  the builder finds identifier-shaped literals of 64 bytes or more — the population that a
  PostgreSQL (or unknown-vendor) deployment now has refused. Raise the `{63,}` to `{128,}` for
  an Oracle-only deployment. Then check the inbox clause separately: `git grep -nE
  'inbox\.tablename|InboxConfig|WithTableName' -- '*.go' '*.yaml' '*.yml'` and measure the
...
- scope: two clauses. (a) Every builder door validates each identifier SEGMENT against the
  ...
  name carrying a bad character reports too-long rather than the character. The cap is PER
  SEGMENT and never on the rendered whole: an 81-byte `schema.table.column` whose segments are
  ...
  every vendor, because its config-time callers have no vendor in scope — and the outbox
  schema-prefix bound is untouched (superseded by [C64.7], which bounds it by the store's
  vendor).
- gate: (a) match — a PostgreSQL or unknown-vendor deployment hands any builder door an
  identifier segment of 64 bytes or more, quoted or bare, written as a literal or computed at
  runtime. no-match — Oracle-only with every segment under 128 bytes, or every segment is short.
  (b) match — a test or caller asserts that an over-long identifier BUILDS (a golden SQL string
  carrying it, a `require.NoError` around such a `ToSQL()`). no-match — nothing asserts on those
  statements. (c) match — a PostgreSQL inbox store is configured with a table name of 50 bytes or
  more, which now fails at startup where it used to be admitted up to 114. no-match — the table
  name fits, or the store is Oracle.
- apply: (a) Shorten the identifier to fit the vendor's cap. Quoting does NOT help — the interior
...
- ref: gaborage/go-bricks#1437 · [ADR-100](adr_100_vendor_identifier_grammar_behind_renderer.md)
  ...
  `database/internal/builder/identifiers.go` (`validateSegment`), `inbox/store.go`. Builds on
  [C64.3], which brought the charset half of the same funnel.

### [C64.6] `keystore/testing.AssertKeyNotFound` refuses a key returned with an error · breaking · when: match

- detect: `git grep -nE 'AssertKeyNotFound\(' -- '*_test.go'` in YOUR tree — the helper ships
  from `keystore/testing`, so the population is your own tests. The open paren anchors on a
  CALL. The grep finds the call sites; what decides the outcome is the `app.KeyStore` under
  test, so read each subject's error paths too — nothing in a build fails here.
- scope: the helper judged absence by error-ness alone and discarded both lookups' returned
  keys. Each returned KEY is now checked as well: a non-nil key fails the test even when the
  lookup also returned an error. The failure message renders the stray key's dynamic TYPE
  (`%T`) and never the key itself. The verbs follow the positional rule of `[C64.4]` — the
  public-key check aborts, the private-key check records. The exported signature is
  unchanged, and `app.KeyStore`'s interface doc now states that a lookup returning an error
  returns no key.
- gate: (a) match — a store under test returns a key ALONGSIDE an error, the natural shape of
  a refresh failure that falls back to a cached key. (b) no-match — every error path of the
  store under test returns `nil`, which is the framework's own keystore, its `MockKeyStore`,
  and every green test today.
- apply: (a) Fix the store so an error path returns no key — that is the contract the helper
  now enforces and the interface doc now states. If the cached-key fallback is deliberate,
  assert it with your own direct `PublicKey`/`PrivateKey` call and check both the error and
  the key you get back. Do NOT wrap the helper or route around it to restore the old reading:
  a wrapper that accepts a key returned with an error re-hides the same gap under a local
  name. (b) Nothing to do.
- verify: `go test ./...` in your tree  # then (a) confirm the store's error paths return no
  key — make one return a ZERO-VALUE or otherwise synthetic `*rsa.PublicKey` deliberately,
  never a live loaded key, and check the helper fails naming a TYPE and not the key's
  contents; (b) a green run is the check
...
- scope: the change ships as a two-link stack — the messaging and `app` half first
  ...
  attempt of that publish carries the instant the publish began; `MessageId` the outbox row id on a RELAYED
  publish, a generated UUID everywhere else. In detail: `DeliveryMode` becomes `amqp.Persistent` on EVERY publish — typed, outbox-relayed,
...
- detect: `git grep -nE 'jose[.]Policy' -- '*.go'` shortlists every file naming the type; the hits
  that matter are the ones where such a value is an operand of `==` or `!=`, or the key type of a
  `map[...]`. Keep the pattern free of `\b`/`\s`/`\w` — `git grep -E` has no PCRE escapes, so a
  pattern carrying one silently matches nothing and the gate reports "not affected". `go vet ./...`
  is the authoritative answer regardless: the compiler resolves this one even through an alias or
  dot-import a line-oriented grep misses, and unlike `go build ./...` it type-checks `_test.go`
  files, where a `==` between two policies is at least as likely as in production code and would
  otherwise pass a green build untouched
- scope: `jose.Policy` (`jose/policy.go`) gains four fields for the new bare-JWE mode — `Mode
  SealMode`, `Typ string`, `ProtectedHeaders map[string]any` and `IATMillis bool` — and the map
  among them costs the struct its comparability: `==`, `!=` and map-key use on `jose.Policy` stop
  compiling. **Unchanged**: assignment, copying, passing by value, struct literals, field access,
  and every existing policy's behaviour — `SealModeJWEofJWS` is the zero value, so a policy that
  never mentions `Mode` seals and opens exactly the bytes it did before, and a `SealModeJWEofJWS`
  policy that sets `Typ`, `ProtectedHeaders` or `IATMillis` is refused at validation with
  `JOSE_POLICY_MODE_MISMATCH` rather than silently changing shape. `reflect.DeepEqual` still
  *compiles* and is not unchanged: it now walks `ProtectedHeaders` and distinguishes a nil map from
  an empty one. `jose.Header` did NOT lose comparability — it gained `Typ string` and `IATMillis
  int64`, both scalars, deliberately
- gate: match = the detect's hits include a `==`/`!=` between two `jose.Policy` values, or a map
  keyed on one. no-match = you only construct, pass and read policies, which is the common case —
  a policy is a configuration input, not a value object, so most consumers never compared them
- before:

  ```go
  if got == (jose.Policy{Direction: jose.DirectionOutbound, SignKid: "our-signing"}) { /* ... */ }

  seen := map[jose.Policy]bool{}
  seen[*p] = true
  ```
...
- after:
  ...
  whole-struct equality — most often in a test assertion — `reflect.DeepEqual` (or
  `require.Equal`/`assert.Equal`, which use it) still compiles and now compares
  `ProtectedHeaders` too, which `==` could never have done
- why: Visa Message Level Encryption prescribes protected headers the framework cannot enumerate
  in advance (`iss` and partner-specific entries beside the framework's own `alg`/`enc`/`kid`), so
  carrying them means carrying a map, and a struct holding a map is not a comparable value. A
  pointer-to-map would restore `==` as pointer identity — two identical policies comparing unequal,
  and code that kept compiling while silently changing meaning — which is worse than a break the
  compiler finds for you, and is the trade `[C59.2]` already refused for the messaging structs
- verify: `go build ./... && go vet ./...`
- ref: gaborage/go-bricks#1575 · [ADR-107](adr_107_jose_bare_jwe_mode.md) ·
  [ADR-058](adr_058_consumer_scoped_amqp_arguments.md) and `[C59.2]` (the same break, same
  reasoning, on `ConsumeOptions`/`ConsumerOptions`/`ConsumerDeclaration`) · `jose/policy.go` ·
  [jose.md](jose.md#bare-jwe-mode-visa-message-level-encryption)
...
- gate: match — a PostgreSQL outbox store is configured with a schema-qualified table name
  ...
  unqualified, or the store is Oracle (128 unchanged).
- apply: inspect the ledger BEFORE renaming, then rename the schema to 63 bytes or fewer.
  PostgreSQL did not refuse the over-long name, it TRUNCATED it at 63 bytes — and `CreateTable`
  interpolates the configured name straight into its DDL, so a deployment running with
  `outbox.autocreatetable` may already hold its ledger in the truncated schema rather than the
  one written in config. Read the schema the table actually lives in (`SELECT schemaname FROM
...
- verify: `go build ./... && go test ./...`  # then boot the service against a PostgreSQL
  ...
  on Oracle confirm a 128-byte schema still boots
- ref: gaborage/go-bricks#1495 · [ADR-100](adr_100_vendor_identifier_grammar_behind_renderer.md)
  and its outbox schema-segment amendment · `outbox/store.go`
  (`validateTableNameForVendor`), `outbox/store_postgres.go`, `outbox/store_oracle.go` ·
  builds on [C64.5]

### [C64.8] the connect seam refuses an empty PostgreSQL host · breaking · when: match

- detect: `git grep -nE 'DBConfigProvider' -- '*.go'` to find your provider implementations,
  then READ each one for a branch that returns a `DatabaseConfig` whose `Host` is empty or
  whitespace-only, with no
  `ConnectionString` — a host that arrives from a config server, a tenant record or an
  unresolved secret is inventoried by hand, because no grep reaches the value. Your TEST
  fixtures are part of the population: a stub `DBConfigProvider` or a hand-built
  `DatabaseConfig` that names `Type: postgresql` and omits `Host` reaches the seam the same
  way production config does, and now fails there — the framework's own suite needed a host
  added in three packages.
- scope: `config.ApplyDatabasePoolDefaults` / `ApplyDatabasePoolDefaultsForKey` now return a
  `MissingFieldError` naming `database.host` (section-qualified, so a tenant reads
  `multitenant.tenants.<id>.database.host`) for a PostgreSQL section whose `Host` is empty
  after trimming — the seam canonicalizes `Host` with `strings.TrimSpace`, so `"   "` counts as
  missing — and no
  `ConnectionString`, where they used to hand the empty host straight to pgx. Under pgx v5.10.0
...
- apply: set `Host` to a non-empty, non-whitespace value on every config your provider returns
  — `localhost` if you were relying on the old TCP dial. A value that is only whitespace (a
  mounted secret that delivered a newline, say) is refused the same way an absent one is. The refusal lands at the first `deps.DB(ctx)` for that tenant,
  which is first USE and not startup, and for `go-bricks-migrate` at config resolution after
  its next pin bump.
- verify: `go build ./... && go test ./...`  # then resolve one tenant through your provider and
  confirm the connection is acquired
- ref: gaborage/go-bricks#1544 ·
  [ADR-050](adr_050_connectionstring_type_inference.md) amendment 2026-09-07 ·
  `config/database_section.go` (`validatePostgreSQLFields`)

### [C64.9] pgx v5.11.0 applies libpq's raw-DSN rules verbatim · no-consumer-action

- note: Only raw `connectionstring` values are affected; the framework-built DSN is unchanged. libpq rules now apply verbatim: in keyword/value form a backslash escapes the next character (`sslrootcert=C:\certs\ca.pem` reads as `C:certsca.pem`; write `C:\\certs\\ca.pem`, quoting does not help); in URI form `+` is literal (write `%20` for a space), `#` is data, the FIRST `@` ends userinfo, and a bare IPv6 host must be bracketed. A rejected DSN surfaces as the constant `failed to parse PostgreSQL config`; `errors.As` to `*pgconn.ParseConfigError` for pgx's redacted detail. Also inherited: `BeginTx` keeps the pooled connection on a recoverable server error, cancelled queries close with FIN, and `user.Current()` is no longer called when a user is supplied.
- ref: gaborage/go-bricks#1540 · pgx CHANGELOG v5.11.0

### [C64.11] the unsealed dedup key falls back to the `message_id` property when the outbox stamp is absent · breaking · when: match

- detect: nothing in your build flags this — no signature moves, and the change is a
  WIDENING, so no call site stops compiling and no existing key changes. The population is
  code that depends on the REFUSAL. `git grep -nE 'DedupKey\(' -- '*.go'` finds every read,
  then read each one for what its error arm does — anything but re-raise puts it in the
  population, in EITHER direction. A handler that branches on
  `errors.Is(err, messaging.ErrInvalidEventID)` to DROP, DLQ or alert on an unstamped delivery
  stops firing that arm for a delivery carrying a `message_id`. The opposite arm is the one
  easier to miss and the bigger semantic move: a handler that fell through the error to
  process WITHOUT dedup was at-least-once for those deliveries and is now exactly-once, so a
  redelivery that used to re-run is now a silent skip+ACK. `git grep -nE 'ErrInvalidEventID' -- '*.go'` catches
  the branches that never mention `DedupKey`. Then inventory the PRODUCERS on every queue a
  typed consumer reads. Two shapes reach the fallback, and a foreign-producer sweep misses the
  second: a queue fed by a foreign or hand-rolled producer, AND a queue fed by your own
  `DeclareTypedPublisher` — the direct typed publish door adds no `x-outbox-event-id` (that stamp
  belongs to the outbox relay) while the AMQP path mints a `message_id` on every publish, so those
  deliveries have always carried exactly the shape this change now dedups on. `git grep -nE
  'DeclareTypedPublisher\(' -- '*.go'` finds them; cross them against the queues your typed
  consumers read. Only an outbox-relayed queue is out of the population, because it always stamps. Neither grep reaches a producer outside your repo — that half is a
  by-hand inventory of who publishes to each queue, and a partner that sets `message_id`
  today has been silently DLQ'd until now, so your DLQ is the better census: search it for
  deliveries carrying a `message_id` and no `x-outbox-event-id`.
- scope: `Metadata.DedupKey()` gains a second source on the UNSEALED branch only. When the
  delivery carries no `x-outbox-event-id` header at all, the AMQP `message_id` property is
  read and validated by `ValidateEventID` — the SAME `^[A-Za-z0-9_-]{1,128}$` grammar the
  header answers to, so `:` stays excluded and a property can no more mint a sealed
  `<SignFamily>:<jti>` key than a header could; ADR-097 §4's shared-ledger suppression
  closure is unchanged. Precedence is stamp-first and does NOT fall through: a stamp that is
  present but empty, over-long, wrongly typed or otherwise malformed still errors, because on
  a go-bricks producer the stamp is framework-written while the property is caller-written
  and a caller must not be able to shadow the stamp by spoiling it. Only a delivery with NO
  stamp key in its header table reaches the property. Both absent is the pre-existing error,
  byte-identical. The sealed branch, `ValidateEventID`, `ValidateDedupKey`,
  `inbox.ProcessOnce`'s signature and the ledger grammar are all untouched, and
  `x-idempotency-key` is NOT a framework dedup source — the glossary reserves the idempotency
...
- gate: match — a typed consumer reads `Meta.DedupKey()` on a queue whose producer sets
  `message_id` without the outbox stamp, whether that producer is foreign or your own
  `DeclareTypedPublisher`. no-match — every producer on every queue you consume is a go-bricks
  outbox relay (always stamped, so the property is never reached), or you compose the ledger key
  yourself instead of calling `DedupKey`, or your consumers are sealed.
  The direct-typed-publisher arm carries a consequence the foreign arm does not: the framework
  mints that `message_id`, and #1546 makes it stable across the attempts of one logical publish, so
  a broker REDELIVERY of such a message now DEDUPS where `DedupKey` used to error. A consumer that
  relied on that error to see every redelivery — a retry counter, an at-least-once side effect —
  changes behaviour without its own code moving.
- apply: decide, per consumer, whether an unstamped delivery SHOULD dedup. If it should —
  the reason this exists — there is nothing to do: the delivery that used to nack into the
  DLQ now processes once. If it should NOT, the refusal is now yours to make: check
  `meta.Headers()[messaging.HeaderEventID]` yourself, or reject on
  `meta.MessageID() != "" && !stamped`, BEFORE calling `DedupKey`, and return your own error
  so the delivery keeps taking the poison path. Do not try to restore the old behaviour by
  narrowing the grammar — it is shared with the header and with the ledger door. Then drain
  what the old behaviour parked: deliveries already in a DLQ were refused for exactly this
  reason and will process on replay, so replay them deliberately rather than discovering
  them later, and remember the ledger has no record of them, so a replay is a genuine first
  processing, not a dedup hit.
- verify: `go build ./... && go test ./...`  # then publish one delivery carrying a
  `message_id` and no `x-outbox-event-id` to a queue your typed consumer reads, and confirm
  it processes once and a redelivery of it is a dedup hit rather than a second run
- ref: gaborage/go-bricks#1547 · [ADR-097](adr_097_sealed_amqp_messages.md) and its
  2026-09-08 unsealed-fallback amendment · `messaging/dedup_key.go` (`Metadata.DedupKey`),
  `messaging/typed_consumer.go` (`Metadata.MessageID`) · widens [C63.2], which brought the
  grammar this fallback reuses

### [C64.12] the dead-letter helper declares both of its queues as quorum queues · topology-change · when: match
...
- detect: `git grep -n 'DeclareQueueWithDLQ(' -- '*.go'` over your own modules. EVERY call
  ...
  call site is OUT of the population on two grounds only: an `x-queue-type` already set on
  that queue's declaration — `messaging.NewQueue(name)` with `Args["x-queue-type"]` before
  `RegisterQueue`, or `d.Queues["<queue>.dlq"].Args["x-queue-type"]` written after the fact
  (the ADR-040 passthrough, which the helper never overwrites) — or a spec that pins the type
  itself, `&messaging.DeadLetterSpec{QueueType: messaging.QueueTypeClassic}`, which is honoured
  on the primary queue and the parking queue alike and so changes neither. Then ask the BROKER what you
  actually have, because the code cannot tell you: `rabbitmqctl list_queues name type` (or the
  management API) for each name the greps produce and for each one's PARKING queue — which is
  `DeadLetterSpec.ParkingQueue` where the call site sets it and `<name>.dlq` only otherwise, so a
...
- gate: match = the broker already holds a classic parking queue (`ParkingQueue` when the spec
  sets it, else `<queue>.dlq`), or a classic PRIMARY queue, declared by this helper. RabbitMQ's declare-equivalence check refuses
  a redeclaration that changes the queue type, so startup fails with `PRECONDITION_FAILED`
  naming the queue. no-match = no `DeclareQueueWithDLQ` call site, or every queue it touches is
  already quorum, or every one of them already carries an explicit `x-queue-type` through
  `Args`.
- apply: Decide per route, BEFORE the bump. To keep today's topology exactly, set the type
  explicitly — `decls.DeclareQueueWithDLQ("orders.queue", &messaging.DeadLetterSpec{QueueType:
...
- gist: session-scoped state — `pg_advisory_lock`, a `SET`, a temp table, `DBMS_SESSION` —
  belongs to one physical backend, and a pool is free to run the next statement on a different
  one, so a lock taken on one statement is released on a connection that never held it while
  both statements succeed. The pinned handle that fixes it (`types.Session` = Querier +
  Transactor + Close, over `*sql.Conn`, with tracking parity and vendor doors) shipped without
  being declared on `types.Interface`, so reaching it meant asserting to an anonymous
  `interface{ Session(ctx context.Context) (Session, error) }` — in consumer code, and through a
  private `sessionOpener` inside the tracking wrapper. `Interface` now declares the method: the
  assertion and the runtime "does not support dedicated sessions" error are gone, `deps.DB(ctx)`
  reaches the door directly, and every implementation outside the framework stops compiling until
  it provides `Session` (C65.5, ADR-112). The framework's own doubles come with it —
  `database/testing.TestDB.ExpectSession()` queues a strict `TestSession` with its own
...
- gist: a consumer whose broker lost its topology across a reconnect retried `ConsumeFromQueue`
  against a `404` forever, logging at Debug. The registry now re-declares its exchanges, queues and
...
- detect: three populations. Static config: `git grep -nE 'connectionstring:' -- '*.yaml' '*.yml'`
  ...
  does; a service (`service=`/`PGSERVICE`) is not one of the four sources this rule reads, and a
  DSN naming one is refused ahead of this rule by `[C66.3]`. Provider implementations:
...
- scope: in `validatePostgreSQLFields`'s `ConnectionString` branch, after the `database.tls`
  ...
  cannot silently drift apart on a pgx bump. Env-sourced TLS claims are `[C66.1]`.
...
- scope: `app.InboxProcessor.ProcessOnce` and `inbox.(*Inbox).ProcessOnce` take
  ...
  sealed key retained past the delivery that minted it still passes, and the ledger then either skips
  the next delivery as a duplicate or records its work under the retained key. Requiring the key to EQUAL the one bound to the
...
### [C65.5] `database.Interface` declares the session door · compile-break · when: match

- detect: `git grep -nE 'MigrationTable\(\) string' -- '*.go'` lists every type you implement
  `database.Interface` (`types.Interface`) on — the migration-table getter is the method nothing
  but an `Interface` implementation has, so it finds the doubles a search for the interface NAME
  misses, including the ones assigned through a function parameter or a struct field. Keep the
  pattern free of `\b`/`\s`/`\w` — `git grep -E` has no PCRE escapes, so a pattern carrying one
  silently matches nothing and the gate reports "not affected". `go vet ./...` is the
  authoritative answer regardless: it type-checks `_test.go` files, where a hand-written database
  double is most likely to live and where `go build ./...` would never look
- scope: `types.Interface` gains `Session(ctx context.Context) (Session, error)`, returning the
  `types.Session` handle (Querier + Transactor + Close) added the hop before. Inside the
  framework, `tracking.Connection.Session` calls the method directly and its private
  `sessionOpener` duck-type is deleted along with the `"%T does not support dedicated sessions"`
...
- gate: match = you implement `database.Interface` / `types.Interface` on a type of your own — a
  test double, an adapter, a decorator that does not embed a framework connection. no-match = you
  only CONSUME the interface (`deps.DB(ctx)`, a `database.Interface` parameter), or every
  implementation of yours embeds a framework connection and inherits the method
- before:

  ```go
  type fakeDB struct{}

  // ... Query/QueryRow/Exec/Prepare/Begin/BeginTx/Health/Stats/Close/DatabaseType/
  // MigrationTable/CreateMigrationTable ...

  var _ database.Interface = (*fakeDB)(nil) // stops compiling
  ```

- after:

  ```go
  // A double that never opens a session says so, rather than handing back a nil handle a
  // caller would dereference:
  func (f *fakeDB) Session(_ context.Context) (database.Session, error) {
      return nil, errors.New("fakeDB opens no sessions")
  }
  ```

  A decorator over a framework connection should forward instead, and wrap the handle if it
  wraps statements: `return f.inner.Session(ctx)`. A double that needs real session behaviour
  can use the framework's own: `database/testing.TestDB.ExpectSession()` returns a strict
  `TestSession` carrying its own query, exec and transaction expectations, popped in declaration
  order, whose every call returns `sql.ErrConnDone` after `Close`; `testing/mocks.MockDatabase`
  takes the session to hand back via `ExpectSession(session, err)`. Keep the `var _
  database.Interface = (*fakeDB)(nil)` assertion in the double's own package — it is what turns
  the next such break into a compile error at the definition rather than at a far-away call site
- why: the handle existed and the door did not, so every consumer reaching a pinned connection
  wrote `db.(interface{ Session(ctx context.Context) (Session, error) })` — an assertion the
  compiler cannot check, which fails at runtime on a spelling drift, and which the framework was
  itself forced to carry internally. The capability is not optional (both vendors and the
  tracking wrapper implement it, and no configuration removes it), so an exported opt-in
  `SessionOpener` interface would have advertised a choice that does not exist while leaving the
  assertion in place. Adding the method to an interface is apidiff-incompatible by construction;
  the population it breaks is hand-written doubles, which the compiler names one by one
- verify: `go build ./... && go vet ./...`
- ref: gaborage/go-bricks#1009 · [ADR-112](adr_112_database_session_door.md) ·
  [ADR-008](adr_008_database_testing_interface_segregation.md) (the `database/testing` design
  `TestSession` follows) · [ADR-032](adr_032_lease_refcount_tenant_handles.md) (a Session holds no
  tenant lease of its own and must not outlive the scope that acquired it) ·
...
- scope: `Validate` refuses any identifier field whose lowercased form is `public` or starts
  ...
  matcher keeps matching and the message still names the field and the value. All three entry
  points that build role statements call `Validate`, so `ProvisionPGRoles`,
  `ProvisionPGRolesTx` and `PGRoleProvisioningSQL` all refuse — the last matters most, since it hands a script to
...
- note: New exported surface, all additive: `DbManager.Remove(key string) error`,
  ...
  `App.CacheManager() *cache.CacheManager`; `DbManager.Stats()` gains `removals` on the `/ready`
  public stats; `cache.ManagerStats` gains `Removals` (also on `/ready`), so an UNKEYED positional
  `cache.ManagerStats{…}` literal stops compiling and the compiler names it (run `go vet ./...`
  rather than `go build ./...`, since such literals usually live in `_test.go`); a KEYED literal
  is unaffected. Semantics and the rotation recipe:
  [database.md](database.md#evicting-one-connection-credential-rotation).
- ref: gaborage/go-bricks#1622 · `database/manager.go`

### [C65.10] a consumer re-declares its topology on each new channel after a reconnect · no-consumer-action

- note: after the AMQP client reconnects, the consumer registry records the new channel generation
  and re-runs its exchange, queue and binding declarations once for it before re-subscribing, so a
  broker that lost topology no longer leaves a consumer in a silent 404 loop; declares are
  idempotent, so a healthy reconnect adds one pass. A declaration refused with
  `PRECONDITION_FAILED` (a surviving entity with different arguments) is logged once at WARN and
  skipped until the process restarts — fix the server-side definition and restart. A re-subscribe
  that keeps failing logs at WARN from its fifth consecutive attempt, with
  `amqp_reply_code`/`amqp_reply_text` when the broker refused it, so an alert on messaging WARN
  volume can fire where it stayed quiet. The pass keys on the unexported `channelGeneration`
  method, so a client re-declares only when its type carries it — the `*AMQPClientImpl`
  `messaging.NewAMQPClient` returns, or a struct embedding it; a client type without it from a
  custom `app.Options.MessagingClientFactory`, such as a wrapper holding an `AMQPClient` in a
  field, keeps the old behavior. No configuration key.
- ref: gaborage/go-bricks#1618 · [ADR-113](adr_113_amqp_topology_redeclare_on_reconnect.md) ·
  `messaging/registry.go`, `messaging/amqp_client.go`

### [C65.11] `messaging.Manager.Stats()` renames `active_consumers` to `consumer_registries` and gains per-consumer subscription counters · silent-behavior · when: match
...
- scope: the key is renamed, not retired. It counted tenant keys in the consumer map — one for a
  ...
  exported surface: `messaging.ConsumerState` with `GivenUp()`, `Registry.ConsumerStates()` and
...
- apply: rename the field in the scrape. A panel labelled as a consumer count was never one — point
  it at `declared_consumers` or `subscribed_consumers` instead, which are.
- verify: `curl -s localhost:8080/ready | jq '.checks[]? // . | .. | .messaging_stats? // empty'`
  names the four keys.
- ref: gaborage/go-bricks#1666 ·
  [messaging.md](messaging.md#consumer-subscription-state-and-stats) · `messaging/registry.go`,
  `messaging/manager.go`, `app/readiness.go`

### [C65.12] `messaging.consumers.critical` fails `/ready` once a consumer gives up re-subscribing · additive-optional
...
- note: New opt-in key `messaging.consumers.critical` (bool, absent = `false`, env
  ...
  `messaging.ManagerOptions.ConsumerResubscribeDelay` (zero keeps the registry's 5s floor). One new
  stats key travels with it: `Manager.Stats()` gains `consumer_max_fail_streak` (int — the largest
  current-outage re-subscribe streak across declared consumers, `0` when none is failing), on the
  `/ready` public-stats allowlist beside `subscribed_consumers`, so the intermediate state is
  observable before the verdict flips. Purely additive to the map; a scrape that does not know the
  key is unaffected. On the Go side, so an
...
- gist: under a sealed delivery's context `ValidateDedupKey` admitted any sealed key, whichever
  envelope composed it, so a handler that kept one past the delivery it came from either skipped the
  next delivery as a duplicate (when the retained key already had a ledger row) or ran it
  and persisted its work under the retained key. The sealed consume door now stores that
...
- gist: a PostgreSQL `connectionstring` naming a libpq service through `service=` or
  `PGSERVICE` used to reach pgx, which merged the service file's host over a TCP `PGHOST`
  and dialed a socket with TLS skipped, or, with no `PGHOST`, was refused as host-less.
  Either shape is now refused ahead of the host rules, naming the service's source
  (C66.3, ADR-050 amendment, #1644).
- gist: `ExchangeDeclaration.Type` was never validated, so an empty, wrong-case or misspelled
  type passed startup and the broker refused the declare on replay. `Validate()` now refuses
  any type that is not one of the four AMQP core types or an `x-` plugin type, naming the
  exchange and the type. `messaging.ExchangeTypeDirect`/`Topic`/`Fanout`/`Headers` and
  `DeclareDirectExchange` are new (C66.4, ADR-116, #1566).

- gist: `MigrateAll` keeps one `Results` row per dispatched tenant, but its result never recorded
  how many tenants were listed, so an empty listing returned a zero-row result with a nil error and
  an empty `Failed()`, which reads as a clean fleet. The result now carries `NeverDispatched` and
...
- gist: `RegisterExchange` had no existence check, so re-declaring one exchange name replaced the
  ...
  or `DeclareDirectExchange` of that same name let registration order pick the broker's type — the
  typed exchange winning dropped every dead-lettered message carrying a routing key, the fanout
  winning fanned the module's own traffic out, and neither failed startup. A compatible repeat now
  merges and an incompatible one keeps the FIRST declaration and fails `Validate()`, exactly as a
  queue re-declaration has since `[C56.7]` (C66.7, ADR-118, #1714).

---

### [C66.1] a unix-socket PostgreSQL connectionstring refuses a `PGSSL*` TLS claim · breaking · when: match
...
- scope: `scanPostgresDSN` records per-key presence for `sslmode`,
  ...
  names EVERY key that claims after the merge, in the scan's key order, each with the exits
  that close ITS arm — unset the variable or shadow it with a non-claiming DSN key, where a
  DSN-carried claim is simply dropped — and then the judged surface. Each claim refuses on
  its own, so clearing one only surfaces the next. Same doors as `[C65.2]`; `go-bricks-migrate` inherits through the exported
  `ApplyDatabasePoolDefaults` seam, no CLI change. A DSN naming a service is refused ahead of
  this rule by `[C66.3]`. `PGPASSFILE`, `PGSSLPASSWORD` and `PGSSLSNI` are out of scope; `PGSSLROOTCERT` is
...
- apply: the claim is in the environment, not in the string — a match REQUIRES the matching
  DSN key to be absent — so there is no DSN key to drop, and dropping one that is present
  would expose the variable it was shadowing. Unset EVERY `PGSSL*` variable the refusal
  names — a non-empty `sslcert`, `sslkey` or `sslrootcert` claims as surely as an `sslmode`,
  so several can be in effect and each refuses alone; or, where the environment is not yours
  to change, ADD a non-claiming value for each of those keys to the DSN (`sslmode=disable`,
  `sslcert=''` — a present key, empty included, wins over env); or point the DSN at a TCP
  host, the one exit that clears them all. A DSN already carrying such a key is not this atom.
- verify: `go build ./... && go test ./...`  # then boot, or resolve one tenant, with every
  source the refusal named cleared — or with them beside a TCP host — and confirm the
  connection is acquired. Clearing only the first named source reproduces the refusal under
  the next one.
...
- detect: nothing in your build flags this — no signature moves. `git grep -nE
  ...
  when the key it passes is SEALED and spells a different `<SignFamily>:<jti>` than the
  delivery being handled.
  Only a seal-tagged `WithMeta` consumer's `meta.DedupKey()` mints a sealed key, so the
  matching shapes are a handler that stashes THAT value in a field, a cache, or a
  variable outliving the delivery and reuses it on a later one, and a call that passes a
  sibling delivery's key. A `messaging.WireDedupKey` value is never sealed and
  `ValidateDedupKey` returns before the equality check for it, so a wire key is still
  admitted under a sealed delivery's context, whatever it spells, and is out of scope.
- scope: `messaging.ValidateDedupKey(ctx, key DedupKey)` still refuses the zero key
  ...
  stream declaration).
- gate: match — a `ProcessOnce` call passes a SEALED key whose `<SignFamily>:<jti>`
  differs from the one the delivery being handled composed: a key retained across
  deliveries, or one read from a sibling delivery's `meta`. `ValidateDedupKey` compares
  the key's VALUE, not which delivery produced it, so a sibling whose envelope repeats
  the same family and `jti` is still admitted. no-match — every sealed key a handler
  passes spells the current delivery's own `meta.DedupKey()`, or the key is a
  `WireDedupKey`, which this atom does not touch.
...
### [C66.3] a PostgreSQL connectionstring that resolves through a libpq service is refused · breaking · when: match

- detect: a PostgreSQL section (or one whose scheme or keyword form infers PostgreSQL) whose
  `connectionstring` carries a `service` key, as a keyword pair or a URI query parameter and
  whatever its value, or carries none while `PGSERVICE` is non-empty in the process environment.
  `git grep -nE 'service=' -- '*.yaml' '*.yml'` shortlists static DSNs; then check `PGSERVICE`
  in every environment that process loads (Compose, Helm, systemd, the operator's shell).
  Provider implementations and the migrate CLI's control-plane target are the same two
  hand-read populations as `[C65.2]`. A DSN that names its own `host` is in the population
  too: the service still fills whatever the DSN leaves unset.
- scope: `scanPostgresDSN` records the presence of the `service` key, and
  `validatePostgreSQLConnectionString` refuses AHEAD of `[C65.2]`'s rules (1) and (2), whatever
  `PGHOST` or a DSN `host=` says, when the DSN carries that key at all, or carries none while
  `PGSERVICE` is non-empty. pgx v5.11.0 merges a service file
  over the environment and under the DSN, so a service's `host=/var/run/postgresql` wins over
  a TCP `PGHOST` and dials the socket with `TLSConfig == nil`; the seam never reads service
  files, so it cannot judge that host, or the port, user and `sslmode` the file supplies. pgx
  resolves a service on the key's PRESENCE, and `pgservicefile` files a `[]` section under the
  empty name, so `service=''`, a trailing `service=` or `?service=` reads that section of the
  default `~/.pg_service.conf` and is refused like any named service — the triage brief had
  accepted an empty key; this atom does not. Note that `service= user=u` names the service
  `user=u`, since pgx skips whitespace after `=`. `servicefile=` or `PGSERVICEFILE` with no service named is inert, as
  ...
  service-only DSN `[C65.2]` refused as host-less (Category `missing`) is refused here instead,
...
- gate: match = a DSN carries a `service` key, or runs under a non-empty `PGSERVICE` with no
  `service` key of its own.
- apply: copy every key the service section sets from its `pg_service.conf` into the
  connection string, drop `service=` from it (an empty one too), and unset `PGSERVICE` wherever it is set: dropping `service=` alone exposes a
...
- verify: `go build ./... && go test ./...`  # then boot, or resolve one tenant, with the
  service inlined and `PGSERVICE` unset, and confirm the connection is acquired.
- ref: gaborage/go-bricks#1644 ·
  [ADR-050](adr_050_connectionstring_type_inference.md) amendment 2026-09-16 ·
  `config/postgres_dsn.go` (`scanPostgresDSN`) ·
  `config/database_section.go` (`validatePostgreSQLConnectionString`, `pgServiceSource`) · same
  seam as [C65.2]

### [C66.4] an exchange whose type is not an AMQP core or `x-` plugin type fails startup · breaking · when: match

- detect: nothing in your build flags this. `git grep -nE 'ExchangeDeclaration|\.Type = ' -- '*.go'`
  shortlists hand-built exchanges: a struct literal, a `var` or `new(...)` declaration, the
  `testing/fixtures.NewExchangeDeclaration` builder, or a `Type` assigned after construction. Read each one's `Type`. A declaration from
  `DeclareTopicExchange`, `NewTopicExchange`, `DeclareDirectExchange`, `NewDirectExchange`
  or `DeclareQueueWithDLQ` is in the population only when its `Type` is reassigned afterwards.
- scope: `Declarations.Validate()` refuses every exchange whose `Type` is not exactly `direct`,
  `topic`, `fanout` or `headers` and does not start with `x-`. The check is case-sensitive,
  and an empty `Type` is refused. Every violation is reported in one error, sorted by
  exchange name, and each line reads `exchange "<name>" has unknown type "<type>"`.
  Such a declaration used to boot, and the broker then refused `exchange.declare` on replay.
  `ExchangeDeclaration.Type` stays `string`. `ExchangeTypeDirect`, `ExchangeTypeTopic`,
  `ExchangeTypeFanout`, `ExchangeTypeHeaders`, `NewDirectExchange` and
  `(*Declarations).DeclareDirectExchange` are additions. Exchange `Args` are not judged.
- gate: match = a hand-built exchange whose `Type` is empty, differs in case from a core
  type, or is misspelled. no-match = every `Type` is a core type spelt exactly or an `x-`
  plugin type.
- apply: set `Type: messaging.ExchangeTypeDirect` (or `Topic`/`Fanout`/`Headers`), or
  replace the literal with `decls.DeclareTopicExchange(name)` /
  `decls.DeclareDirectExchange(name)`. A plugin exchange keeps its `x-` type.
- verify: `go build ./... && go test ./...`  # then boot and confirm startup passes
  declaration validation. A remaining match fails with `exchange "<name>" has unknown type`.
- ref: gaborage/go-bricks#1566 · [ADR-116](adr_116_exchange_type_validation.md) ·
  `messaging/declarations.go` (`validateExchangeTypes`), `messaging/helpers.go`

### [C66.5] a fleet migration run carries a three-way verdict; an empty listing is not clean · breaking · when: match

- detect: `git grep -nE 'migration\.MigrateAll\(|MigrateAllResult\{' -- '*.go'` finds every run and
...
- scope: `migration.MigrateAllResult` gains `NeverDispatched []string`: the listed IDs the run
  ...
  listed and every listed tenant was dispatched and none failed. It returns `migration.ErrFleetSplit` when at least one tenant was
  dispatched and at least one failed or was never dispatched. It returns
  `migration.ErrNothingAttempted` when no tenant was dispatched: an empty or nil listing, a context
  already done, quiesce set before the first dispatch, or a nil result, which is what `MigrateAll`
  returns for a nil argument or a listing failure. A dispatched tenant is failed whatever stopped
  it (config resolution, `ErrFlywayTimeout`, `ErrFlywayCanceled`, a fail-fast cancel). A parallel
  run whose context is already done now dispatches no tenant and returns the context's error. It
  used to dispatch a random prefix, and under fail-fast its error was then the first dispatched
  tenant's failure. Both runners also re-check the context after the quiesce check, so a cancel
  landing during a check that fails open no longer dispatches the next tenant. The parallel runner
  checks only the context before waiting for a worker slot and judges context and quiesce once it
  holds one, so a fail-fast cancel or a quiesce flag set while it waited no longer starts the next
  tenant. A parallel run both canceled and quiesced returns the context's error where it returned
  `ErrQuiesceBlocked`, matching the sequential run.
  Unchanged: `Results` still holds one row per dispatched tenant, and the quiesce pins on its
  absence stand. `Failed()` still returns only failed rows, and its godoc now says so. The
...
- apply: gate the run on `res.Verdict()` beside the error, for example `if v := res.Verdict(); v !=
  ...
  touched. Re-run the tenants in `res.Failed()` and `res.NeverDispatched`; never infer the latter from `Results`. Key any unkeyed
  literal (`MigrateAllResult{Action: …, Results: …}`).
- verify: `go vet ./... && go test ./...`  # then run once against an empty listing and confirm the
  caller's gate reports `migration: no tenant attempted` instead of success.
...
### [C66.7] an incompatible exchange re-declaration keeps the first declaration and fails startup · breaking · when: match

- detect: nothing in your build flags this. `git grep -nE 'RegisterExchange\(|DeclareTopicExchange\(|DeclareDirectExchange\(|DeclareQueueWithDLQ\(|DeclarePublisher\(' -- '*.go'`
  in your app, then read the hits for one exchange **name** reached from two places. Two
  different modules is the usual shape, and it is exactly the case no single call site can see —
  so grep the names, do not trust a per-module reading. `DeclareQueueWithDLQ` is a declaration
  site for an exchange you did not name: it registers `DeadLetterSpec.Exchange`, or
  `<queue>.dlx` when the spec leaves it empty, as a **fanout**, so add those derived names to the
  list before comparing. `DeclarePublisher(opts, exchange)` is a declaration site too: it now
  registers the exchange it is handed whatever is already stored, so a non-nil `exchange`
  argument counts as one of the two places. The grep sees only your own tree, so a shared or
  vendored module that declares an exchange can supply the second call site invisibly —
  when you compose one, rely on `verify` rather than a clean grep. A name that appears once
  is not a match; a name that appears twice is a match only if the two shapes disagree (see
  gate). Nothing changes for a name declared once, which is the overwhelming majority.
- scope: `Declarations.RegisterExchange` gains an existence check. A repeat declaration of one
  name merges when `Type` and the four flags (`Durable`, `AutoDelete`, `Internal`, `NoWait`) are
  equal and every `Args` key the two share carries the same value; the union of their `Args` is
  what reaches the broker, so the stored shape no longer depends on which call ran first. An
  incompatible repeat keeps the incumbent, mutates nothing — not even an `Args` key the
  incumbent never had — and records a conflict `Validate()` reports, which
  `ModuleRegistry.DeclareMessaging` wraps as `declaration validation failed: %w` and which
  aborts startup. Fields are compared in a fixed order, returning at the first difference:
  `Type`, `Durable`, `AutoDelete`, `Internal`, `NoWait`, then `Args` keys in sorted order.
  `Type` leads because it is what the broker routes on. Repeats of one disagreement collapse, so
  the reported count enumerates distinct problems. `Clone()` carries the conflicts, so a cloned
  declaration set cannot pass a validation its source failed. No signature moves; the runtime
  `Registry.RegisterExchange` replay sink is unchanged, as is `RegisterBinding`, which still
  appends. Supersedes `[C56.7]`'s "Exchanges are unchanged and still last-write-wins", which
  recorded the E56 hop accurately.
- gate: three outcomes, not two. match (conflict) = one exchange name declared twice where the
  two declarations disagree on `Type` or on any of the four flags, or where both set the same
  `Args` key to different values (compared with `reflect.DeepEqual`, which is **type-sensitive**:
  `int(1)` and `int64(1)` disagree). Startup now aborts; take the LAST paragraph of `apply` and
  align the two call sites. The likeliest one by far is a `DeclareQueueWithDLQ` DLX name —
  fanout, durable — also declared through `DeclareTopicExchange`, `DeclareDirectExchange` or
  `DeclarePublisher`, which differ in `Type`. match (union) = one exchange name declared twice
  where the two agree on `Type`, on the four flags and on every `Args` key they SHARE, but one of
  them sets an `Args` key the other never sets. Startup passes and nothing is reported, yet this
  is not a no-op either: the stored declaration becomes the UNION whichever call carried the
  extra key, so the `exchange.declare` reaching the broker changes shape and a broker already
  holding that exchange with different arguments refuses it with `PRECONDITION_FAILED`. Do not
  file this under no-match — take the FIRST paragraph of `apply`, which decides between dropping
  the added `Args` and a controlled migration. no-match = every exchange name is declared once,
  or the repeats are IDENTICAL — including several primary queues sharing one DLX through
  `DeadLetterSpec.Exchange`, which merges to exactly what was already stored. Note that a repeat
  declaration is no longer a silent no-op you can ignore: before this bump the later call
  replaced the earlier one wholesale, so an app could be *relying* on the overwrite without
  knowing it.
- apply: a repeat that only ADDS `Args` the other never sets is not a conflict, but it is not a
  no-op either: the stored declaration is now the union, where it used to be whatever the last
  call carried, so the `exchange.declare` that reaches the broker changes. That matters only
  where the broker already holds the exchange AND the arguments it holds DIFFER from the merged
  set: a redeclare whose arguments match what is already there is compatible and passes. Where
  they differ it is refused with `PRECONDITION_FAILED`, which
  [ADR-113](adr_113_amqp_topology_redeclare_on_reconnect.md) skips until the process restarts.
  Prefer dropping the added `Args` at the call site that introduced them: where the argument is
  not actually required that clears the redeclare outright, with no topology surgery and no
  window in which messages can be lost. Where it IS required, migrate under control — do **not**
  delete and recreate the exchange in place as a quick fix. `exchange.delete` takes the
  exchange's BINDINGS with it: while it is absent every publish to it fails, and a publish made
  after it is recreated but before its bindings are restored is unroutable and silently
  DISCARDED, because AMQP's `mandatory` flag defaults to `false` and nothing reports the drop.
  Two safe shapes. (1) In place, with a publish pause: quiesce the producers publishing to that
  exchange and let the bound queues drain, record every binding on it (`rabbitmqctl
  list_bindings`, or the management UI's export), delete it, redeclare it with the union `Args`,
  restore every recorded binding, and only then let the producers resume. (2) Side by side, no
  publish pause: declare a **new** exchange name carrying the union `Args`, bind the same queues
  to it, cut the producers over to the new name in one coordinated deploy, and delete the old
  exchange only once it is idle and nothing is bound to it. For a real conflict: align the call
  sites so the two declarations agree — pick the intended `Type` and flags and the intended
  value for the contested `Args` key, and make both call sites say it. For the DLX collision,
  the fix is almost always to stop declaring the DLX by hand and let
  `DeclareQueueWithDLQ` own it, because the parking queue is bound with an **empty routing key**
  and only a fanout delivers to it: a topic or direct DLX silently dropped every dead-lettered
  message that carried a routing key. If you need a typed exchange under that name, give the
  dead-letter exchange a different one through `DeadLetterSpec.Exchange`. If your two
  declarations genuinely need different shapes, they are different exchanges — give them
  different names.
- verify: boot the app and confirm startup does not abort with `declaration validation failed:
  conflicting exchange declarations` — the error names every conflicting exchange, the field or
  `Args["<key>"]` at fault, and both values labeled by which one is in effect
...
- ref: gaborage/go-bricks#1714 · [ADR-118](adr_118_exchange_redeclaration_conflicts.md) ·
  ...
  [wiki/messaging.md](messaging.md#helper-functions-for-simplified-declarations) · same
  mechanism as [C56.7]

## E67 · v0.66.0 → v0.67.0 — `go-bricks-migrate` exits 0/1/2 by run verdict, and its summary record counts what was listed, attempted and never attempted + a publisher-only registry re-declares its topology per reconnect and once per pooled publisher

- gist: the CLI had two exit codes, so "the fleet is split" and "nothing was touched" both exited 1,
  and an empty tenant listing exited 0 — a deploy gate read that as a clean fleet. It now exits by the
  verdict ADR-115 gave the library in `[C66.5]`: 0 clean, 1 split, 2 nothing dispatched, the pre-loop
  failures included, and every misuse — an unknown flag, a stray argument, a flag combination that
  does not resolve — lands on 2 as well, so exit 1 means a split fleet and nothing else. The `--json`
  summary record keeps `total` and gains `listed`, `attempted`, `not_attempted` and `verdict`, and is
  emitted on every exit-2 path that reaches an action (C67.1, ADR-115 amendment, #1692). The CLI is its own
  module, tagged `tools/migration/vX.Y.Z` after the framework release, so this atom applies when the
  pipeline's `go-bricks-migrate` binary moves to the tag cut from this hop — not when the library
  bump alone lands.
...
- scope: the CLI's `migrate`, `validate` and `info` actions exit **0** when the run was clean, **1**
  ...
  (`clean`, `fleet_split` or `nothing_attempted`) beside `event` and `action`. The text summary names
  the verdict and the four counts; `total` is a JSON-only key, as it has always been. Every invocation that reaches `migrate`/`validate`/`info` emits exactly one
...
- apply: a step that treated exit 1 as "the fleet may be split" must now also handle 2 for the
  misuse it used to see as 1 — a typo in a flag moves 1 → 2. Then decide what a zero-tenant
  environment means for that pipeline. A new environment that may
  legitimately have no tenants must accept exit 2 explicitly (`code=$?; [ "$code" -eq 0 ] || [ "$code"
  -eq 2 ] || exit "$code"`); a rollout gate that must never run against an unconsidered fleet keeps
  treating 2 as a failure, which is the point of the code. Replace `.total` with `.attempted` (the
  dispatched count it always held) or with `.listed` where the intent was the fleet size — `.listed`
  is what the run observed, so it is 0 when the listing itself failed. `.verdict` names the FLEET and
  the exit code names the RUN: they coincide except where a run error accompanies a consistent fleet
  (a parallel run cancelled after its last tenant finished reads `clean` and exits 1), so branch on
  the exit code for "did the command succeed" and on `.verdict` for "what state is the fleet in".
...
### [C67.2] a publisher-only registry re-declares per reconnect, and once per pooled publisher · silent-behavior · when: always

- detect: nothing to grep — the change is in the framework's own wiring, so every service that
  declares AMQP topology and publishes gets it. It is VISIBLE where channels rotate often (a
  broker rolling restart, a proxy that cycles connections) and the deployment is multi-tenant,
  because the pass count scales with tenants times pooled publishers.
- scope: the pass had one driver, the consumer supervisor's re-subscribe loop, so a registry
  holding declarations but no consumers never re-declared at all. The registry's own client and
  every publisher client the manager pools under its key now drive it, and the once-per-generation
  guard is keyed per SOURCE, so each source earns a pass on every generation it is first seen on.
  A client drives the pass only if it IS, or embeds, what `NewAMQPClient` returns: the two seams
  are unexported, so a custom `MessagingClientFactory` product that merely holds an `AMQPClient`
  is never a source and gets no publisher-driven redeclare.
  A pooled publisher is a new source on creation and again on every RE-creation after an LRU
  eviction or the idle sweep retired the last one.
- gate: always. There is nothing to match on: the extra passes ARE the repair, and they arrive
  with the upgrade.
- before: a broker restart cost one declare pass per registry that had consumers, and none at all
  for a publisher-only one — which is the bug: that service kept publishing into topology the
  broker had lost until someone restarted the process.
- after: it costs one pass per source — T registries × M pooled publishers — each replaying every
  exchange, queue and binding that registry declared, serialized behind that registry's pass mutex
  and run off the publish path.
- apply: nothing to change. Size it if your fleet is large: the work is (E+Q+B) declare
  round-trips per source. Declares are idempotent for matching arguments, so a pass that finds
  nothing lost costs round-trips and changes nothing at the broker, and a declaration refused with
  `PRECONDITION_FAILED` is still skipped by every later pass until the process restarts.
- verify: delete an exchange under a live connection on a publisher-only service and publish — the
  message reaches the queue without a process restart, which is what the extra passes buy.
- ref: gaborage/go-bricks#1761 ·
  [ADR-113](adr_113_amqp_topology_redeclare_on_reconnect.md) amendment ·
  `messaging/manager.go` (`observePublisherChannels`, `registryFor`) ·
