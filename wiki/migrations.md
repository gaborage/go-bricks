...
- detect: `git grep -nE 'outbox\.tablename|OutboxConfig|TableName' -- '*.go' '*.yaml' '*.yml'`
...
- apply: rename the schema to 63 bytes or fewer BEFORE the bump. The name was never usable
  on PostgreSQL: the server truncates an identifier at 63 bytes, so the ledger was already
  landing in a truncated schema, and with `outbox.autocreatetable` on it could be created
  there. WHEN it fails depends on the deployment, because store construction is lazy: a
  static-source, single-tenant-or-shared-ledger service constructs the store during `Init`
  and so fails at STARTUP, while a dynamic config source or a per-tenant multi-tenant
  deployment constructs on first use and fails at the first publish or relay poll — earlier
  than before, since the refusal now lands before any statement is built, but not at boot.
...
- ref: gaborage/go-bricks#1495 · [ADR-100](adr_100_vendor_identifier_grammar_behind_renderer.md)
  ...
  Mirrors [C64.5]'s inbox clause; #1503 folds the two into one helper.
