...
- scope: `migration.MigrateAllResult` gains `NeverDispatched []string`: the listed IDs the run
  ...
  landing during a check that fails open no longer dispatches the next tenant, and a parallel
  run both canceled and quiesced returns the context's error where it returned
