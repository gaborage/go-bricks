...
- gist: The `go-bricks-migrate` CLI now validates every tenant's resolved database config the
  ...
  name every column at most once by vendor identity — two keys folding to one Oracle column built a
  MERGE with a duplicate alias and no error — and on Oracle every key must be a single column name,
  since the USING clause names each one as a column alias (ADR-071, C60.11).
...
- preflight: if you run `go-bricks-migrate`, `go-bricks-migrate info` per tenant before the bump
  ...
  sites whose column maps are assembled dynamically (C60.11)
...
- scope: `database/internal/builder/` only, as two preconditions at `BuildUpsert` (`helpers.go`:
  ...
  The name check runs before the identity check, which is what leaves `columnIdentity`'s
  position-0 quote test unreachable from this API — see ADR-071 for why that guard is bypassed
...
- verify: exercise your Oracle upsert paths and read the errors, since nothing surfaces at build
  time. Two probes are enough: `BuildUpsert("t", []string{"k"}, map[string]any{"k": 0, "id": 1,
  "ID": 2}, nil)` now returns `insert columns must be distinct: "ID" and "id" name the same
  column for upsert` where it used to return a MERGE that Oracle answers with ORA-00957; and a
  key like `"t.name"` or `"count(*)"` now returns `... is not a single column name for upsert`
  where it used to return a MERGE with an alias Oracle cannot parse. Both produce no SQL.
