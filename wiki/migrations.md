...
- preflight: if you run `go-bricks-migrate`, `go-bricks-migrate info` per tenant before the bump
  ...
  ; and `git grep -nE '\.As\(' -- '*.go'` — every `Columns.As` argument that is not a
  bare-identifier literal now panics at the call, so read each one for a parameter, a
  concatenation or surrounding whitespace (C60.28)
