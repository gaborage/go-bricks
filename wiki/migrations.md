...
- preflight: if you run `go-bricks-migrate`, `go-bricks-migrate info` per tenant before the bump
  ...
  (C60.16); and grep your log calls for field names containing `key`,
  ...
  set to nothing (`FOO=`, an empty `secretKeyRef`, an `envsubst` over an unset variable), because
  a numeric key delivered empty now fails startup instead of decoding as `0` (C60.15)
...
### [C60.15] a numeric config key delivered empty fails startup · breaking · when: match
...
- gate: match = any go-bricks variable can reach the process set-but-empty. Read that
  wider than it sounds: for most numeric keys `0` means "use the default", so an empty
...
- ref: [ADR-074](adr_074_delivered_empty_numeric_config.md) ·
  ...
  `internal/configdecode/configdecode.go` (`EmptyStringToNumericGuardHookFunc`) ·
