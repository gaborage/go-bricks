...
- detect: ask the SERVER, because both names are normally derived from a tenant id at
  ...
  ```sql
  -- schema half: is one of these a schema YOU provisioned as a tenant's?
  ...
  **Expected baseline noise**: the built-in `public` schema carries a PUBLIC `USAGE` grant on
  ...
  the schema half at all: judge that half by the provisioning spec and by the built-in
  schema's own existence, ownership and contents (`\dn+ public`, and whether your tenant's
  tables live there), never by the presence of that row. Likewise the catalog
  ...
  **Which path provisioned matters, and a clean grant detect is NOT a no-match.** Those four
  ...
  error and the script carried on into the grants. The Go helper leaves nothing at all.
  `buildPGRoleStatements` (`migration/roles.go:280-289`) appends BOTH roles' create-and-lockdown
  pairs before the first `CREATE SCHEMA` or `GRANT`, and `ProvisionPGRoles`
  (`migration/roles.go:229-235`) executes the list one statement at a time,
  non-transactionally, returning on the first error. `CREATE ROLE "public"` raises
  `reserved_name` (42939), and the DO block's `EXCEPTION WHEN duplicate_object OR
  unique_violation` (`migration/roles.go:331-337`) swallows only those two conditions — not
  42939 — so the block re-raises and provisioning aborts. A `RuntimeRole: "public"` deployment
  that used `ProvisionPGRoles` therefore never created the schema and never ran a single
  grant: it reads clean under all three catalogs AND under the schema query. For that path the
  source read is the only route, exactly as for `MigratorRole` below. Never read a clean detect
  as a no-match without first establishing which of the two paths provisioned the tenant.
...
- gate: match = the schema detect shows a tenant schema of yours that is `public`,
  ...
  reserved name to either role, in any letter case — corroborated by the grant detect for a
  `RuntimeRole` spelled `public` on the script path, by the `pg_roles` detect for a case
  variant the server accepted, and by the source read alone for `MigratorRole` and for the Go
  helper's aborted path, neither of which leaves a server-side trace. no-match = every tenant
...
- apply: (a) fix the names BEFORE the bump; the framework will not do it for you, because
  ...
  targets a different default-ACL row and the `default_acl` finding survives. Then rerun
