...
- detect: nothing in your build flags this. `git grep -nE 'ExchangeDeclaration\{|NewExchangeDeclaration\(|\.Type = ' -- '*.go'`
  shortlists hand-built exchanges: a struct literal, the `testing/fixtures.NewExchangeDeclaration`
  builder, or a `Type` assigned after construction. Read each one's `Type`. A declaration from
