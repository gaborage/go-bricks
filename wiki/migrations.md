...
## E61 · v0.60.0 → v0.61.0 — the server's own 400 details stop echoing request input
...
- verify: both directions. Assert the expressions you intend to ship still build (`err == nil` from
  ...
  `errors.Is(err, dbtypes.ErrDangerousAlias)`. Checking only that good expressions still build passes
  whether or not the validation runs.
- residual: the alias check is a denylist, not a grammar. An alias carrying none of the six sequences
  still renders verbatim — `Alias: "a, (SELECT password FROM users) b"` is accepted by `qb.Expr()` and
  therefore by every door. This hop makes the two construction paths agree; it does not narrow what
