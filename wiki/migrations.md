...
### [C61.25] the outbox ledger is sequenced, laned and drained by one leader · silent-behavior + compile-break · when: match

- detect: two independent sweeps, because this atom breaks at COMPILE time for one audience and
  changes RUNTIME behaviour for another.
...
- scope: four columns join the ledger — `seq` (a per-ledger identity assigned at insert),
  ...
  Two behaviours change for every deployment, including single-tenant ones that configure
...
- gate: match = `outbox.enabled: true` anywhere (schema and behaviour), OR `outbox.Store` is
