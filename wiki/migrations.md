...
- gist: Two of the framework's own 400 details echoed caller-controlled text. `FieldError.Value`
  ...
  than three (C61.8). The JoinFilter value doors render a nil
  operand as `IS NULL` and a slice as `IN (…)` like their Filter siblings, while the ordering
  doors refuse both (C61.11).
...
### [C61.11] `jf.Eq` renders nil and slices like `f.Eq` · silent-behavior · when: match
...
- verify: `jf.Eq("u.id", nil)` renders `u.id IS NULL`; `jf.Eq("u.id", []int{1,2})` renders
