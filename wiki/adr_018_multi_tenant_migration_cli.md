...
Flyway connects with the secret's `username` and `password` as stored, so a fleet
run without a migrator identity needs the migrator's own username and password in
