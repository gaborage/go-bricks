...
# → Provide a COMPLETE database section (type + host + port + database, or
#   connectionstring) — a partial one now fails startup (see ADR-003, ADR-047)
...
#   password/connectionstring marks the section as intended, and an intended
#   section must be complete. Complete it, or remove the block entirely
```
