...
> **The edge still has to hold up its end.** `Host` is itself a request header. If your ingress accepts any `Host` for a wildcard vhost, a caller can send `Host: other-tenant.api.example.com` and the subdomain resolver will read `other-tenant` — the default order removes the *header* override, it does not make the subdomain unspoofable. Have the ingress validate `Host` against the tenant's DNS name (and, with `proxies: true`, ensure only the trusted proxy can set `X-Forwarded-Host`). If `header` participates in `order` at all, the gateway should strip or overwrite inbound `X-Tenant-ID`. Tenant IDs are shape-validated and must resolve to a known tenant, so a forged value fails closed — but that's the second line of defense, not the first.
