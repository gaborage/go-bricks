...
> `auth.Middleware` shape (ADR-109). No config key is added. Mounting the guard makes the same
> posture assertion as `enabled` for that route family, in every environment, whatever `enabled`
> says.
