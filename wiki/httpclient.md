...
net/http guarantees is empty (`1xx`, `204`, `304`, any reply to `HEAD`).
...
this replacement, exactly as before this composition existed; only the tuning
fields (proxy, dialer, pool limits) are new — and `Build()` reports this case as
an error (see below) rather than silently discarding it.
...
below. **None of this applies to `WithTLSConfig(nil)`**, which returns
...
after `WithTLSConfig` with a replacement that does **not** decide its own TLS —
either an opaque (non-`*http.Transport`) `RoundTripper`, or a `*http.Transport`
carrying neither meaningful `TLSClientConfig` material nor a TLS dialer —
...
```go
// myTLSConfig must itself carry meaningful security material — a certificate,
// pinned roots, a version floor. An empty or ALPN-only *tls.Config does NOT
// suppress the discard, and this chain would fail Build().
replacement := &http.Transport{TLSClientConfig: myTLSConfig}

// Succeeds: replacement decides its own TLS, so Build reports no discard — but
// myTLSConfig is what the client actually uses; tlsCfg (passed to WithTLSConfig
...
material would still be replaced, so `Build()` fails outright instead. A signer
that wraps another `RoundTripper` is opaque, cannot be cloned, and so would be
discarded wholesale — also a `Build()` failure. The interceptor path sidesteps
...
of its own; carrying a client certificate, pinned roots or a TLS dialer means that material
would be replaced, so `Build()` fails outright instead. A signer wrapping another
`RoundTripper` is opaque, cannot be cloned, and would be discarded wholesale — also a
`Build()` failure.
...
| Transport error (no response) | `codes.Error`, description = error.type (or the error's Go `%T` when error.type is empty) | yes — type-only event |
| Non-5xx response + non-nil err (interceptor/build failure on a 2xx/3xx/4xx) | `codes.Error`, description = error.type (or the error's Go `%T` when error.type is empty — never its message). Note: 5xx responses always take the row-3 path regardless of whether err is non-nil — `codes.Error`, description `"HTTP {code}"`. | yes — type-only event |
...
**Type-only exception events.** Where the table says "yes — type-only event," the framework records the exception through `observability.RecordErrorByType`: one `exception` event carrying `exception.type` = the error's outer `%T`, and NO `exception.message` ([ADR-083](adr_083_span_sinks_record_errors_by_type.md)). An exception event and a span status description leave the platform with the tracing exporter, and the error reaching them is not framework-written — Go's stdlib `*url.Error.Error()` includes the full request URL with query string (Go redacts userinfo passwords but not query strings), a response interceptor formats whatever it read, and a caller-supplied `RoundTripper` can put anything there. The framework used to export a query-stripped message, which closed the leak that had been found rather than the class; it now exports no message at all. The classified error-type label (`transport_error`, `interceptor_failed`, …) is framework vocabulary and stays the status description where one exists. The full message is still on the log line. Consumers adding their own span attributes carrying header or body bytes must run their own redaction — the rule covers the framework's own sinks only.
...
>
> **PCI/PII workloads must extend the default sensitive-field list.** The framework ships defaults like `password`, `token`, `api_key`, `authorization`, plus card-data names like `cvv` and `otp`, but bare-PAN-style fields not already covered by the defaults (`pan`, `ssn`, `tax_id`, …) need to be added via `log.sensitivefields` in YAML or `app.Options.LoggerFilterConfig` in code before enabling payload logging — see [observability.md](observability.md#sensitive-data-filtering) for the full field list and customization seams.
