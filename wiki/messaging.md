...
The decode summary's field path is gated on the payload type, not on the error: a decoder reports a *map* destination's path with the input key in it (`limits.<key>`, from Go 1.27), and a `json.Unmarshaler` field's path with whatever that method decoded into. So if a map, an `any`, or a `json.Unmarshaler` — `time.Time` included — is reachable from `T`, the summary drops the path and reports only the wanted type and byte offset. A `T` free of all three keeps the field name.
