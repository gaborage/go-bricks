...
**Delivery metadata.** `DeclareTypedConsumerWithMeta` / `messaging.NewTypedHandlerWithMeta[T](eventType, fn)` are the metadata-carrying siblings of `DeclareTypedConsumer` / `NewTypedHandler`: `fn` is `func(ctx context.Context, payload T, meta messaging.Metadata) error`, with the same decode → validate → `fn` pipeline and failure semantics. `Metadata` exposes read-only accessors — `Headers() amqp.Table`, `EventType() string`, `Redelivered() bool`, `DedupKey() (messaging.DedupKey, error)`, `MessageID() string` and `Sealed() (SealedEnvelope, bool)` — so a typed consumer can take the ledger key and wrap its body in `inbox.ProcessOnce`, the canonical composition for an outbox-fed consumer. `MessageID` returns the AMQP `message_id` property for a handler's own policy about an unstamped delivery; it is not a `DedupKey` replacement, and only `DedupKey`'s answer belongs at the ledger door. A `messaging.DedupKey` is a value carrying which door produced it — `String()` is its persisted spelling, `Sealed()` says whether the sealed typed door composed it — and the zero value is invalid. On a plain typed consumer `DedupKey` returns a WIRE key holding the `x-outbox-event-id` header once it passes the ledger grammar `^[A-Za-z0-9_-]{1,128}$`; when the delivery carries no such header at all it falls back to the AMQP `message_id` property under that same grammar, so a producer that follows the standard without being go-bricks is still dedupable. A header that is PRESENT but malformed is an error rather than a fall-through — the stamp is framework-written and the property caller-written, so a caller must not be able to shadow the stamp by spoiling it — and an error wrapping `messaging.ErrInvalidEventID` is returned when both sources are absent or the chosen one is malformed. `inbox.ProcessOnce` takes the `messaging.DedupKey` itself, not a string, and judges it by PROVENANCE rather than spelling: it refuses the zero key, and refuses a sealed key outside the sealed delivery's own context. The grammar is not re-run there — a wire key passed it at construction, in `DedupKey` or in `messaging.WireDedupKey(id)`, the exported door for an id you source or compose yourself (it applies the grammar, wraps `ErrInvalidEventID` on refusal, and never yields a sealed key). `Sealed` answers per-consumer type: a plain typed consumer gets `(zero, false)` for every delivery, whatever the publisher wrote.
...
`Registry.ConsumerStates()`, and `Manager.ConsumerStates()` across every key, returns the state behind those counters per consumer — queue, `Subscribed`, `Resubscribes`, `LastResubscribeAt`, `FailStreak` — in declaration order. `FailStreak` counts the current outage's consecutive failed re-subscribe attempts and the next success resets it; `ConsumerState.GivenUp()` is true once an unsubscribed consumer's streak reaches the fifth attempt, the same threshold that escalates the re-subscribe log to WARN. A consumer declared without a handler never subscribes and never reads as given up. A stopped registry reports every consumer unsubscribed with no streak, so a shutdown is never mistaken for an outage; the cumulative counters survive it.
