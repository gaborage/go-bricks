...
TLS material — not a discard — **provided `custom`'s own `TLSClientConfig` (if any) carries
no security material**. `WithTLSConfig` always overwrites `TLSClientConfig` unconditionally,
so an incumbent that already had its own (a client certificate, pinned roots) still loses
that material to the new `*tls.Config`; the transport-tuning fields compose, but the TLS
material does not, and that case remains a reported displacement even though the clone
itself succeeds.
...
(`httpclient/tls.go`) instead tests for actual security-relevant fields — certificates,
`RootCAs`, `InsecureSkipVerify`, `MinVersion`, `ServerName`, cipher suites, and the
`VerifyPeerCertificate`/`VerifyConnection` hooks — and ignores `NextProtos` entirely, so the
...
itself a `*http.Transport` whose `TLSClientConfig` carries meaningful security
material (`tlsConfigCarriesMaterial` — the same predicate `baseTransportForTLS`
uses, see Determinism below), the predicate is suppressed — the caller did exactly
what the message asks. This is deliberately **not** a nil check on `TLSClientConfig`:
...
examples — must handle the second return value. `NewClient(log logger.Logger) Client`
...
When `custom`'s `TLSClientConfig` carries no meaningful security material (nil, or the
ALPN-only shape `Clone()` can leave behind), `WithTransport(custom).WithTLSConfig(cfg)`
now produces a client that actually has both `custom`'s proxy/dialer settings and `cfg`'s
TLS material, where before it silently lost the former. This fixes real,
previously-unaddressable configurations, not just the error-reporting path around them.
When `custom`'s `TLSClientConfig` DOES carry its own security material, the tuning still
composes but the TLS material is still replaced — and `Build` still reports that as a
displacement, exactly as before this change.
