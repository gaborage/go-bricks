...
> **Amendment (2026-09-18):** the multi-tenant isolation mechanism decided here — a separate Redis database per tenant — will be superseded by the key prefix proposed in [ADR-116](adr_116_cache_key_namespace_and_cluster_mode.md), which ships with the `cache.redis.keyprefix` change under #1727. Until it lands, isolation is still by separate database; from then on, read it as by key prefix, plus a separate database where the deployment supports one.
>
