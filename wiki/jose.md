...
- **Strict algorithm allowlist**: `RS256`/`PS256` for signing; `RSA-OAEP-256` + `A256GCM` for encryption on this nested path — `A128GCM` is admitted too, but *only* behind `Policy.Mode: jose.SealModeBareJWE` (see [Bare-JWE mode](#bare-jwe-mode-visa-message-level-encryption)). `alg=none`, `HS*`, `RSA1_5`, and `ES256` are rejected at parse time. Bodies are **compact serializations only** — go-jose also accepts the JSON form and tolerates whitespace inside a compact, and the framework refuses both at every door, since RFC 7515 §7.1 and RFC 7516 §7.1 admit neither; whitespace *around* a compact is still trimmed, so a token read from a file with a trailing newline opens — and every compact the framework opens, at either layer and in every mode, must carry an encoded protected-header segment of at most 16 KiB — the base64url text as it arrives, measured before any decode, so the decoded JSON header it admits is about three quarters of that. A larger one is refused before it is base64-decoded, as `JOSE_MALFORMED` on the outer layer and `JOSE_INNER_NOT_JWS` on a nested inner JWS — the same codes those layers already use for any unparseable body, so the size refusal is not distinguishable on the wire. The refusal class is API-visible via `errors.Is` on the error chain to framework-internal callers, and is deliberately not logged. go-jose imposes no header-size bound of its own, and the only other ceiling is the 10 MiB body cap, which a negative `MaxResponseBytes` can disable on the client transport. ECDSA support is gated on extending `keystore.KeyStore` to return ECDSA keys (tracked in [#347](https://github.com/gaborage/go-bricks/issues/347))
