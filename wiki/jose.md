...
**Reach**: bare mode is reachable from `jose.Seal` / `jose.Open` (and `jose/testing`'s `SealForTest` / `OpenForTest`, which call them), and from `httpclient.Builder.WithJOSE` for outbound calls — `Build()` skips the `SigAlg` default on a bare-mode policy, so the pair above passes validation as written. There is still no `mode` key in the `jose:` struct-tag grammar, so inbound server routes cannot select it. Visa MLE's `{"encData":"<compact>"}` body envelope is carried by the `httpclient` hooks `WrapBody`/`UnwrapBody` and the ready-made `httpclient.VisaMLEEnvelope()` pair — see [httpclient.md](httpclient.md#jose-body-envelopes-visa-message-level-encryption).
