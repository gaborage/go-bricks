...
namespaces every key as `<prefix>:<key>`, defaults to `app.name`, folds a tenant in so its stored
keys are `<prefix>:<tenantID>:<key>`, and is applied as a `cache.Cache` decorator above whichever connector is in
...
**Date:** 2026-09-03 | **Status:** Accepted | **Breaking:** none at adoption — sealing is additive and import-gated; the header-id grammar it relies on is `[C63.2]`, the typed-door removal ADR-096's `[C63.1]`; amended 2026-09-08 (#1547): the unsealed dedup key falls back to the `message_id` property, `[C64.11]`; amended 2026-09-12 (#1558): the ledger key is a typed `messaging.DedupKey` carrying its provenance, `[C65.7]`; amended 2026-09-16 (#1634), BREAKING: a sealed key must equal the one bound to the delivery in hand, so one whose `<SignFamily>:<jti>` differs is refused before the ledger row and the handler, `[C66.2]`
