...
proxy/dialer settings *and* gets `cfg`'s TLS material), and the case-1 discard predicate is
suppressed when the displacing transport is itself a `*http.Transport` whose `TLSClientConfig`
carries meaningful security material (`tlsConfigCarriesMaterial`, not a bare non-nil check —
...
call-site error and a startup-time construction error instead of a runtime surprise;
