...
arm FIRST: any declared consumer whose supervisor has given up — unsubscribed with its consecutive
re-subscribe failures at `consumerResubscribeWarnFromAttempt`, the same threshold that escalates the
log to WARN — fails the probe, before the publisher's `IsReady()` is read. The threshold is what
