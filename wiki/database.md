...
**Operand contract (`f.Eq`/`f.NotEq` and `jf.Eq`/`jf.NotEq`):** a `nil` operand renders `IS NULL` / `IS NOT NULL`, a slice OR ARRAY expands to `IN (…)` / `NOT IN (…)`, and an empty one renders the constant `(1=0)` / `(1=1)` — the underlying builder classifies both kinds as lists, so `[3]int{1,2,3}` behaves like `[]int{1,2,3}`. A `[]byte` is a scalar, not a list. Both doors RESOLVE the operand before classifying it, so a typed nil pointer (`(*int)(nil)`) and a `driver.Valuer` reporting NULL (`sql.NullString{}`) are nil for equality as well: they render `IS NULL` / `IS NOT NULL`, not a placeholder comparison. A Valuer HOLDING a value stays a scalar — but `jf` binds the value it RESOLVES to (`int64(5)`), not the wrapper (`sql.NullInt64{5,true}`), at all six of its compare doors — equality and ordering alike, while `jf.In`/`jf.NotIn`/`jf.Between` are untouched; the SQL is unchanged, the argument list is not, and the driver receives the same value either way. For a SCALAR operand `jf.NotEq` renders `!=` where `f.NotEq` renders `<>` (#1200); every other shape matches. Prefer the explicit spelling — `f.In`/`f.Null` and `jf.In`/`jf.Null` — when you know the shape: it says what you mean and does not depend on a runtime type test. The ordering methods (`Lt`, `Lte`, `Gt`, `Gte`) refuse `nil`, a slice and an array at both doors, because neither has a rendering for them: `jf` reports `dbtypes.ErrOrderingOperandNotComparable` (exported from `database/types`, so `errors.Is` works from your own code) at `ToSQL()`, `f` reports the underlying builder's own messages, which `errors.Is` cannot match. A `dbtypes.RawExpression` operand (`qb.Expr(…)`) is spliced before any of this — its SQL goes into the predicate verbatim, with no placeholder and no argument — so it is neither resolved nor classified. For every other operand: after resolution — a `driver.Valuer` becomes its `Value()`, a pointer becomes its element, and a NIL pointer is nil before either is asked — the JoinFilter compare doors classify the operand as NIL (`nil`, a typed nil pointer, a Valuer reporting NULL), a LIST (a slice or an array), or a SCALAR (anything else, `[]byte` and a Valuer holding a value included); equality renders nil and list, ordering refuses nil and list, and a scalar is bound to `col op ?` whatever its Go type — a struct no driver accepts is passed through, not diagnosed. Filter classifies the same three ways but resolves differently at the ordering doors and on a nil pointer, so read the divergences below before assuming the doors agree.

The two ordering doors agree on `nil`, on a slice or array, and on a `driver.Valuer` reporting NULL — all refused at both. They diverge on ONE shape: a typed nil pointer (`(*int)(nil)`, the ordinary spelling of an optional field). `jf` dereferences it and refuses; `f` does not dereference, so it renders `col < ?` and binds the nil pointer, which the driver sends as NULL and which therefore matches nothing. Tracked in #1205; `jf.Between` is a third spelling again, reporting the underlying builder's message rather than the sentinel. They diverge on a second shape at EVERY door, equality included: a nil POINTER to a Valuer type (`(*sql.NullString)(nil)`) satisfies `driver.Valuer` through the value receiver, so asking it for its value dereferences nil — squirrel asserts the interface before it tests the pointer and PANICS, and so do `f`, `jf.In`, `jf.NotIn` and `jf.Between`. Only the six `jf` compare doors settle the nil pointer first, rendering `IS NULL` / `IS NOT NULL` for equality and `ErrOrderingOperandNotComparable` for ordering. Guard that operand anywhere else; tracked in #1209. One consequence worth stating plainly: at the `jf` compare doors an UNGUARDED nil operand now MEANS `IS NULL` rather than misfiring, so a nil check you keep around such a call is no longer a workaround — it is the only way left to say "emit no predicate at all". Delete it only if you meant `IS NULL`.
