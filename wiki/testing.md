...
`cache/redis/client_cluster_integration_test.go` is the plaintext worked example and `client_cluster_tls_integration_test.go` the TLS one. The latter's certificate covers `localhost` and nothing else ON PURPOSE: the slot map advertises the announced IPv4, and a certificate covering both names would be accepted whether or not go-redis reuses the seed's pinned SNI for every node it discovers, which is the property the test exists to prove. Three guards hold that premise up — `requireLeafCoversOnlyPinnedName` that the certificate excludes the advertised address (exact `DNSNames`, no IP SANs), `requireSlotMapAvoidsPinnedName` that the fixture still advertises an address the certificate excludes, and `requireSlotMapAvoidsSeedAddress` that the advertised address also differs from the seed address the client was handed, since otherwise go-redis reuses the seed connection and no discovered-node dial happens at all.
