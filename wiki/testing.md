...
`containers.StartRedisTLSContainer(ctx, t, cfg, caPEM, certPEM, keyPEM)` (and the `MustStartRedisTLSContainer` wrapper) boots the same image as `StartRedisContainer` with the plaintext listener disabled and TLS bound to 6379, so `Host()`/`Port()` address the TLS listener unchanged and only the client config differs. Client certificates are not required, which keeps a negative test failing on server verification rather than on a missing client cert. Mint the material with `gbtesting.CAAndServerCertPEM(t, hosts...)` (`testing/certfixtures.go`), which returns a throwaway CA plus a leaf signed by it, SANs split into DNS names and IP addresses; with no hosts it covers `localhost`, `127.0.0.1` and `::1`. The container's `Host()` is only known after start, so a test mints for those three up front and skips if Docker hands back something else. `cache/redis/client_tls_integration_test.go` is the worked example: a verified handshake plus round-trip, a plaintext client refused by the TLS-only server, and unknown-authority and hostname-mismatch failures.
