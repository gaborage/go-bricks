...
Separately from the reaper, each helper's configured `StartupTimeout` now binds the wait itself at every level — both the composite's deadline and each leaf strategy's own timeout (Oracle 120s; PostgreSQL, Redis and RabbitMQ 60s). Previously the composites were bounded with `MultiStrategy.WithStartupTimeout`, which sets the composite deadline but leaves every leaf unbound, so each leaf fell back to testcontainers' 60s default — and for the three module-based helpers that composite was itself wrapped by `WithWaitStrategy`'s hardcoded 60s deadline. `testing/containers/wait.go` holds the mechanism and the vendor behaviour it works around.

### Redis over TLS

TLS is a property of the Redis container, not a second starter: set `RedisContainerConfig.TLS` to a `containers.RedisTLSMaterial{CA, Cert, Key}` and every existing starter (`StartRedisContainer`, `StartRedisContainerForTestMain`, `MustStartRedisContainer`) boots the same image with the plaintext listener disabled and TLS bound to 6379, so `Host()`/`Port()` address the TLS listener unchanged and only the client config differs. A nil `TLS` (the default) stays plaintext. Client certificates are not required, which keeps a negative test failing on server verification rather than on a missing client cert. Mint the material with `gbtesting.CAAndServerCertPEM(t, hosts...)` (`testing/certfixtures.go`), or `NewCAAndServerCertPEM(hosts...)` where there is no `testing.TB` — a `TestMain` starter, say; either returns a throwaway CA plus a leaf signed by it, SANs split into DNS names and IP addresses, defaulting to `gbtesting.DefaultCertHosts()` (`localhost`, `127.0.0.1`, `::1`). The container's `Host()` is only known after start, so a test mints for those up front and skips if Docker hands back something else. `cache/redis/client_tls_integration_test.go` is the worked example.

### Redis behind ACLs

Credentials are a property of the Redis container too: set `RedisContainerConfig.ACL` to a `containers.RedisACL{Admin, App, AdditionalUsers}` — `containers.RedisACLUser{Username, Password, Rules}` values — and every starter boots the same image with the implicit `default` user **disabled** and those identities installed as `--user` boot arguments. `AdditionalUsers` installs further named identities alongside `App`, each with its own `Rules`, which is how one container serves a second credential whose grants differ. Every username must be non-empty, distinct from the others, and not `default`; the starter refuses the set otherwise, because a directive Redis rejects at boot is echoed verbatim into a container log testcontainers dumps to the test output. The password in that directive is installed as a SHA-256 rule (`#<digest>`), never the clear text — the server hashes what a client AUTHs with and compares, so `Config.Password` stays the clear-text value, which is also what the fixture's own `redis-cli` execs pass as `--pass`. Only throwaway passwords belong in these fields for that reason. A nil `ACL` (the default) leaves the stock open server, so no existing caller changes. It composes with `Cluster` and `TLS` the same way those two compose with each other — each arm contributes its own flags to one accumulated command line — and the fixture's own cluster bootstrap and `CLUSTER INFO` readiness execs then run as `Admin` (unauthenticated they would be refused, and the only symptom would be a readiness timeout). One builder renders those execs, so a container that is cluster AND TLS AND ACL gets the CA-pinned transport flags and the admin credential on the same `redis-cli` invocation.

Disabling `default` is the point rather than a detail: left enabled it is `nopass ~* +@all`, so an unauthenticated client works and an authentication test passes whether or not the credential ever reached the wire. `Admin` exists for the fixture's chores (`+@all`); `App` is the credential under test, and the rules it names are whatever the test's own subject needs. Those rules are emitted *before* the generated `resetpass on #<digest>`: Redis applies rules left to right, so a flag-like rule that would disable authentication — `nopass`, `off`, `reset` — is overridden by the `on` that follows it (`reset` also drops the grants before it, which nothing restores, so that identity authenticates and then fails loudly with `NOPERM`), while a caller's own password rule (`>pw`, `#hash`) is not overridden at all but *appended* to the identity's password list, which is what the `resetpass` clears. Either way an identity this fixture installs accepts exactly the password it was declared with, and nothing else. A user naming no `Rules` gets the fixture's narrow default — `+ping +info +get +set +del +eval +command +cluster|slots` over `~*` — so the lazy path is a narrow user rather than an invented one; it holds neither `CONFIG` nor `FLUSHDB`, so an ACL-gated test isolates by deleting the keys it uses instead of flushing. `cache/redis/client_acl_integration_test.go` is the worked example, and derives its own grant list there.
...
### Redis cluster mode, and cluster over TLS

`RedisContainerConfig.Cluster` starts the node with `--cluster-enabled yes` and gives it all 16384 slots, so one server answers for the whole keyspace and a cluster-protocol client has a single endpoint to talk to — the client-side shape an ElastiCache Serverless deployment requires. It is NOT that service's topology: a real Serverless endpoint is sharded across multiple nodes and issues `MOVED` redirects to route a client between them ([ADR-117](adr_117_cache_key_namespace_and_cluster_mode.md)), and this fixture reproduces neither. The fixture then announces the mapped host address, because the slot map carries each node's OWN address and a container-side one is unreachable from the test.
...
`cache/redis/client_cluster_integration_test.go` is the plaintext worked example and `client_cluster_tls_integration_test.go` the TLS one. The latter's certificate covers `localhost` and nothing else ON PURPOSE: the slot map advertises the announced IPv4, and a certificate covering both names would be accepted whether or not go-redis reuses the seed's pinned SNI for every node it discovers, which is the property the test exists to prove. Two guards hold the halves of that premise — `requireLeafCoversOnlyPinnedName` that the certificate excludes the advertised address (exact `DNSNames`, no IP SANs), and `requireSlotMapAvoidsPinnedName` that the fixture still advertises an address the certificate excludes. A third, `requireSlotMapAvoidsSeedAddress`, holds a premise those two do not touch: that the advertised address also differs from the SEED address the client was handed. go-redis keys node clients by address string, so where the two collapse onto one string it reuses the seed connection and there is no discovered-node dial to verify — the SNI question above stays answerable, but nothing exercises it on a second node.

One node owning every slot is not a sharded topology: MOVED redirects, per-shard version skew and partial-shard outages stay uncovered. MOVED is the one worth naming, because it is not merely absent from the fixture — the real target actively produces it. [ADR-117](adr_117_cache_key_namespace_and_cluster_mode.md) records both halves: ElastiCache Serverless shards (a hash tag would pin every key of a deployment to one slot and defeat that sharding), and a standalone client dialling its configuration endpoint fails on the first `MOVED`. A one-container fixture can stage neither. `--tls-cluster yes` is in the same bucket — a single node never dials a peer over the bus, so the flag is required for a real cluster and unexercised by this suite; `redisOptions` records what it changes.
...
| --- | --- | --- |
| `MUTATE_CPU` | 6 | Core budget for `make mutate`: it bounds test execution exactly, while build phases can overshoot it because compile processes nest one level. A per-worker share and an effective worker count are derived from it together, so `workers x share` never exceeds it — a budget below `MUTATE_WORKERS` shrinks the worker count rather than handing each worker a core. The share is pinned as `GOMAXPROCS`/`GOFLAGS -p` on every child. Negative values are rejected; `0` opts out. |
