...
- Handler errors and panics → message nacked WITHOUT requeue (no infinite retry loops). Make handlers thread-safe and idempotent; declare `Args["x-dead-letter-exchange"]` on the queue to park failures instead of dropping them (build with `messaging.NewQueue`, set `Args`, then `decls.RegisterQueue(q)` — `RegisterQueue` deep-copies `Args` at registration; see [wiki/messaging.md](wiki/messaging.md)).
