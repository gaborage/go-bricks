...
- **Wait for CI with one backgrounded watcher, never a burst of foreground polls.** Launch a single `run_in_background` command that exits once the run reaches a terminal state, then keep working — the harness re-invokes you when it exits. Read status with `gh run view --json status,conclusion` or `gh api repos/{owner}/{repo}/commits/{sha}/check-runs`; `gh pr checks --watch` false-fails when a dependent job is skipped, and repeated GraphQL status queries burn the rate limit. Poll remote state no faster than every 30s.
