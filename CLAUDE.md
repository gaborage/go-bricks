...
- **Stacked PRs for large features (mandatory above ~400 LoC):** any feature whose diff would exceed ~400 changed LoC ships as a stack via the `/gh-stack` skill — never one big PR. Decompose by dependency + logical unit (core API → integrations → docs/wiring); each PR must be self-contained: builds, passes `make check`, the pre-push gates and `make mutate` on its own, and is reviewable without later PRs. The base PR targets `main`, each successor its predecessor; `gh stack submit` sets both and links the Stack, and GitHub retargets to `main` as ancestors merge. Merging is bottom-up by the maintainer (admin-gated — never self-merge); after each merge, `gh stack sync --prune`. Independent changes are NOT a stack — give them separate branches off `main`.
