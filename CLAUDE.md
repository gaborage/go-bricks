...
The session model (Opus 5.5) is the orchestrator, not the worker. Its context is the scarcest resource in the session; spend it only on decomposition, judgment calls, synthesis, and talking to the user. Delegate everything else to subagents via the Agent tool, picking the cheapest model that can do the job well:
...
- The orchestrator itself only edits directly when the change is small (one or two files, already-known locations).
- **Tripwire (added after the orchestrator did a 5-file change inline, 2026-08-26): before the first Edit/Write, count the files the change will touch. Three or more, or any screenshot/browser-proof chore: stop and spawn agents instead. Inline orchestrator work is only sequential diagnosis (each command depends on the previous answer) and 1-2 file edits.**
...
- Raw-SQL escape hatches (`f.Raw()`, `jf.Raw()`, `database.Raw()`, an UPDATE `SetExpr()`, a STRING predicate passed to `Having()`, and every `RawExpression` SQL body — `qb.Expr()`, `qb.MustExpr()` or a struct literal) require an inline `// SECURITY: Manual SQL review completed - <what was verified>` annotation at every call site; one grep finds them all (`git grep -nE 'f\.Raw\(|jf\.Raw\(|database\.Raw\(|SetExpr\(|Having\(|MustExpr\(|[.]Expr\(|RawExpression\{'` — squirrel's own `Expr` inside the builder, and the `Expr`/`MustExpr` doors themselves (`database/types`, the builder, `testing/mocks`), are plumbing, not a construction). The rationale should name the property checked: identifier quoting, value-side parameterization, no user-input concatenation; it may sit above an enclosing dispatch when the property is uniform. `RawExpression.Validate()` never inspects the SQL body.
...
- A type implementing `logger.Redactor` (`RedactedForLog() any`, value receiver) is logged by the filter as its returned shape at `Interface`/`WithFields`, nested values included (never at `Err`, where `ErrorRedactor` is the seam), still needle-filtered, and a sensitive key still masks it whole (ADR-110).
...
- **app/** — Application framework and module system; `app.Options.PostRegisterRoutes` receives every route the App registered (probes one descriptor per method) after the duplicate-route check and vetoes startup before the listener opens ([startup_defaults.md](wiki/startup_defaults.md#route-table-hook))
...
- **jose/** — Nested JWE-of-JWS protection on HTTP request and response bodies; `Policy.Mode` also selects the encrypt-only `SealModeBareJWE` (Visa MLE, ADR-107) and the encrypt-then-sign `SealModeJWSofJWE` (Visa Token Service Issuer, ADR-111 — outer JWS header fixed by the mode, `A256GCM` only, and Visa needs an explicit `SigAlg: PS256` because the default is RS256)
...
Test function names are camelCase (`TestUserServiceCreateUser`, not `TestUserService_CreateUser`); the `check-test-conventions.sh` PostToolUse hook flags violations.
