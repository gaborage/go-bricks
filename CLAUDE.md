...
- **messaging/** — AMQP client for RabbitMQ. `messaging/streams` consumes RabbitMQ streams over the native protocol (5552): offsets committed server-side only after successful handling, handlers inline/sequential, SAC; single-tenant only ([streams.md](wiki/streams.md), ADR-059).
