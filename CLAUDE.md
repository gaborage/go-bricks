...
- **Delivered-empty `debug.allowedips` (ADR-078):** an empty STRING (`DEBUG_ALLOWEDIPS=`, `allowedips: ""`) fails configuration resolution — it used to wipe the loopback default and, with `debug.bearertoken` set, skip the IP whitelist while booting. An empty LIST (`allowedips: []`) stays legal as the deliberate token-only clear; the other `[]string` keys are unchanged.
