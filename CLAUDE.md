...
- **Database absence vs misconfiguration (ADR-047):** the `database:` block is all-or-nothing. Omitting it entirely is a supported posture — `/ready` reports `not_configured` and returns 200 (it used to return a permanent 503, as did *every* static multi-tenant deployment, which now reports `per_tenant`). Setting *any* identity field (`type`, `host`, `port`, `database`, `username`, `password`, `connectionstring`) marks the section as intended, and an incomplete intended section now **fails startup** instead of loading silently and failing at first query. Absence is decided at config resolution scoped to the default `""` key (`config.TenantStore.DBConfig`), never in `database.NewConnection`, which is key-blind. A multi-tenant deployment whose default `""` key does not resolve reports `per_tenant` and consequently carries no critical database probe; one that *does* configure a root block (a shared-ledger control plane) is still probed and still gates readiness. Modules needing a database should implement `app.DatabaseRequirer`.
