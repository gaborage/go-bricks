...
**Operations:** `Get`, `Set`, `GetOrSet` (atomic SET NX), `CompareAndSet` (Lua CAS), `CompareAndDelete` (Lua CAD), `Marshal`/`Unmarshal` (CBOR), `LoadThrough[T]` (the read-through path — bounded cache leg, single-flight; [wiki/cache.md#load-through-reads](wiki/cache.md#load-through-reads)). Per-tenant cache instances managed automatically (LRU eviction, idle cleanup, singleflight).
