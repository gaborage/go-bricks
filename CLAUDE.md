...
- **Log filter walks arrays; needles normalize (ADR-079):** a JSON body shaped `{"…":[{…}]}` no longer panics the log path at either door (`Interface()`, `WithFields`); a slice whose elements the filter rewrites emits as `[]any` (serialized output unchanged). Needle trim/drop-empty/dedup moved into `NewSensitiveDataFilter`, so `Options.LoggerFilterConfig` — which replaces the whole config — can no longer ship one empty needle that masks every field; the masking-disabled WARN now judges the effective list.
