...
- Raw-SQL escape hatches (`f.Raw()`, `jf.Raw()`, `database.Raw()`, an UPDATE `SetExpr()`, a STRING predicate passed to `Having()`, and every `RawExpression` SQL body — `qb.Expr()`, `qb.MustExpr()` or a struct literal) require an inline `// SECURITY: Manual SQL review completed - <what was verified>` annotation at every call site; one grep finds them all (`git grep -nE 'f\.Raw\(|jf\.Raw\(|database\.Raw\(|SetExpr\(|Having\(|MustExpr\(|[.]Expr\(|RawExpression\{'` — squirrel's own `Expr` inside the builder is plumbing, not a construction). The rationale should name the property checked: identifier quoting, value-side parameterization, no user-input concatenation; it may sit above an enclosing dispatch when the property is uniform. `RawExpression.Validate()` never inspects the SQL body.
