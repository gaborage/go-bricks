...
- **Stacked PRs for large features (mandatory above ~400 LoC):** a feature whose diff exceeds ~400 changed LoC ships as a stack via `/gh-stack`, never one big PR. Decompose by dependency + logical unit; each PR is self-contained: builds, passes `make check`, the gates and `make mutate` alone, and is reviewable without later PRs. **Every PR in the stack targets `main`** — `ci-v2.yml` filters `pull_request` on `branches: [main]`, so a successor based on its predecessor gets NO CI while GitHub reports it mergeable. `gh stack submit` chains bases, so retarget successors to `main` and reopen to fire CI. Merging is bottom-up, maintainer-only (admin-gated); after each merge, `gh stack sync --prune`. Independent changes are separate branches off `main`, not a stack.
