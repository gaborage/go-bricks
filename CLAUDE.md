...
- **A blocking question costs a fixed minute before its content matters.** Measured locally, a one-question `AskUserQuestion` stalls ~59s at the median and a three-or-four-question one ~114s — roughly double the wait for roughly quadruple the answers. So decide routine calls yourself and carry on, and when you genuinely must ask, batch the open decisions into one call (the tool takes four) rather than draining them one at a time across the session.
