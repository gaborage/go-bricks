...
- A type implementing `logger.Redactor` (`RedactedForLog() any`, value receiver) is logged by the filter as its returned shape at every door and depth, still needle-filtered, and a sensitive key still masks it whole (ADR-110).
