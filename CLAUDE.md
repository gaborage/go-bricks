...
- **Delivered-empty numeric and bool config (ADR-074, ADR-077):** an empty string bound to a numeric or bool key (`FOO=`, empty `secretKeyRef`) fails startup naming the key instead of decoding as `0`/`false` — which flipped `database.pool.keepalive.enabled` off and un-stricted `cache.critical`; explicit values, unset, YAML omission and YAML null are unchanged.
