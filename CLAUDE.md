...
- **Consumer-scoped AMQP args (ADR-058):** `ConsumeOptions`/`ConsumerOptions`/`ConsumerDeclaration` gain `Args` (`x-stream-offset` for stream queues via `DeclareStreamQueue`), so a map field makes all three non-comparable — `==` and map-key use stop compiling.
