...
CLI releases are tagged `tools/migration/vX.Y.Z` (first: `v0.38.0`; latest: `v0.65.0`). Once a tag exists:
...
go install github.com/gaborage/go-bricks/tools/migration/cmd/go-bricks-migrate@v0.65.0
...
both variables; `list` needs no credentials at all.
