...
2. **Raw-SQL escape hatch audit**: Search for all `f.Raw(`, `jf.Raw(`, `database.Raw(`, UPDATE `SetExpr(`, string-predicate `Having(` and `RawExpression` SQL-body (`qb.Expr(`, `qb.MustExpr(`, `RawExpression{` literal) call sites (the FilterFactory / JoinFilterFactory escape hatches, the Execute Helpers' hand-written-SQL adapter, the argument-carrying SET expression, the string HAVING form, and every expression body, which `Validate()` never inspects) with `git grep -nE 'f\.Raw\(|jf\.Raw\(|database\.Raw\(|SetExpr\(|Having\(|MustExpr\(|[.]Expr\(|RawExpression\{'` (skip squirrel's own `Expr` plumbing inside `database/internal/builder`) and verify each has an inline security annotation matching the prefix `// SECURITY: Manual SQL review completed - ` (the rationale after the prefix is per-site). Flag any call site missing the prefix. `database.Raw` deserves at least as much scrutiny as the other two: `f.Raw`/`jf.Raw` produce a WHERE/JOIN fragment inside a builder that still validates its other identifiers, whereas `database.Raw` replaces the **whole statement**, bypassing the builder's identifier validation entirely. The annotation may live directly above the call, or above an enclosing dispatch (e.g. a table-driven loop or a multi-call JoinOn chain) when the safety property is uniform across the calls. See CLAUDE.md "Detailed Security Guidelines" and the godoc on `FilterFactory.Raw` for the policy.
