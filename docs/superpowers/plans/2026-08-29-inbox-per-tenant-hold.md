...
11b. **The park and drain backoffs reuse the delivery package's saturating series.** `delivery.backoffFor` already doubles and saturates rather than wrapping (C1); the drain's per-tenant backoff calls ONE exported helper there rather than growing a second copy of the arithmetic, and carries an overflow-boundary case of its own.
