...
- Modify: `app/interfaces.go` (`holdLedgerProvider interface { HoldLedger() streams.HoldLedger }`, `holdReplayerSetter interface { SetHoldReplayer(func() streams.HoldReplayer) }`), `app/app.go` (`RegisterModule` — capture the provider into `a.holdLedgers []holdLedgerProvider` and call `SetHoldReplayer(...)` — `*Manager` does not implement `HoldReplayer` until Task 9, so in this task the source returns a plain nil rather than a guarded `a.streamsManager`: a typed-nil `*Manager` would satisfy the interface while implementing none of it, and the guard the plan first described cannot help because the manager does not compile into the interface yet. Task 9 makes the source return the manager. C2 therefore also shipped `assertHoldIsDrainable` in `app/streams_setup.go`, which REFUSED a configured hold while `*streams.Manager` did not implement `HoldReplayer` — a hold nothing can drain is a permanent hold, and C2 had to be safe to ship standalone. Task 9 REMOVED that guard and its two tests as part of making the manager a replayer, and the source now returns the manager behind the nil check that keeps a typed-nil out of the interface), `app/streams_setup.go` (`prepareStreamConsumers` — `Hold: a.holdLedger()`, the first non-nil `HoldLedger()` among providers; two providers → startup error)
