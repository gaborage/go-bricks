...
Adding an identity column backfills existing rows in creation order on both vendors (PostgreSQL ≥ 10, Oracle ≥ 12c), so the pending backlog keeps its old `created_at` order after the `ALTER`.
