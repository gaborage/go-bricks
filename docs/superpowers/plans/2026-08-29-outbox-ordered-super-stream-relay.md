...
- Modify (ADR-088 and the `[C61.25]` atom already EXIST — B1 carries them, because a `feat(outbox)!:` cannot land on main undocumented; extend them for the stream leg rather than creating them): `wiki/adr_088_outbox_ordered_leader_relay.md`
...
- Modify: `wiki/migrations.md` (E61 hop row: count +1, gist clause, extend the existing `[C61.25]` atom and its gist/preflight clauses for the stream leg — B1 already added them, so the count does NOT move again)
...
- `config.OutboxConfig` gains a slice and `outbox.Store` a method: both are compile-breaks for
  code outside the framework (`[C61.25]`); the schema gains four columns, a companion leader
...
- [ ] **Step 5: `make check`, commit** — `docs(outbox): extend ADR-088 and the [C61.25] atom for the stream lane`.
