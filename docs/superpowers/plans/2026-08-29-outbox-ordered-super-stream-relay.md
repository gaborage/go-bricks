...
6. **Key-ordering key.** Every key is namespaced by the scope it orders within, because parking is head-of-line blocking and two rows sharing a string but not a destination must not block each other: `stream:<stream>:<partition_key>` on the stream lane — rows on different super streams do not park each other for carrying the same tenant's key, and a stream row partitioned by `acme` is not confused with an AMQP row stamped `acme`; `amqp:tenant:<stamp>` when an AMQP row carries the tenant stamp (`x-tenant-id` in its persisted headers), deliberately spanning that tenant's exchanges; and `amqp:<exchange>:<routing_key>` otherwise — the destination the row is actually published to, since the routing key alone would collapse two rows aimed at different exchanges onto one key. A row parks its key's later rows in the cycle only on `outcomeFailed`; a dead-lettered row is terminal and an unrecorded row was delivered, so neither parks.
...
- Produces, in `relay.go`:
  ...
  - `runRelayLoop(ctx, log, db, msgClient, lead Leadership, records)`: `parked := map[string]struct{}{}`; per record: `if ctx.Err() != nil { break }`; `if err := lead.Probe(ctx); err != nil { log.Warn()...; res.leadershipErr = fmt.Errorf("%w: ...", ErrNotLeader, err); return res }` — leadership loss is kept OFF `outageErr` and is NOT wrapped by `brokerUnavailableErr`, because its cause is the database and reporting it as a broker outage points an operator at the wrong system; `relayTenant` returns it directly and the next tick re-leads. Headers are decoded ONCE per record here and passed to both `relayKey` and `publishRecord`; `key := relayKey(&records[i], headers)`; `if _, p := parked[key]; p { res.parked++; continue }`; then today's switch, plus `case outcomeFailed: res.failed++; parked[key] = struct{}{}`.
  ...
  - `func relayKey(record *Record, headers map[string]any) string` — headers are decoded ONCE per record by the caller and passed in. `LaneStream` → `LaneStream + ":" + record.Stream + ":" + record.PartitionKey`; otherwise, if `x-tenant-id` is a non-empty string in those headers, `LaneAMQP + ":tenant:" + stamp`, else `LaneAMQP + ":" + record.Exchange + ":" + record.RoutingKey`. A decode failure returns `record.RoutingKey` (the row itself is poison and `publishRecord` dead-letters it; parking by routing key keeps later rows of the same key behind it until it parks — consistent with a permanently failing head).
...
### Task 9: AMEND ADR-088 and the migrations atom, and write the docs

**Files:**

- Modify (ADR-088 and the `[C61.23]` atom already EXIST — B1 carries them, because a `feat(outbox)!:` cannot land on main undocumented; extend them for the stream leg rather than creating them): `wiki/adr_088_outbox_ordered_leader_relay.md`
- Modify: `wiki/architecture_decisions.md` (the `### [ADR-088: …]` entry and the `through ADR-088` counter are already there from B1 — leave them unless the stream leg changes the summary)
...
- Modify: `wiki/migrations.md` (E61 hop row: count +1, gist clause, extend the existing `[C61.23]` atom and its gist/preflight clauses for the stream leg — B1 already added them, so the count does NOT move again)
...
- `config.OutboxConfig` gains a slice and `outbox.Store` a method: both are compile-breaks for
  code outside the framework (`[C61.23]`); the schema gains four columns, a companion leader
...
- [ ] **Step 5: `make check`, commit** — `docs(outbox): extend ADR-088 and the [C61.23] atom for the stream lane`.
