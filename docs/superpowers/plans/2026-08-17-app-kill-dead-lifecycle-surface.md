...
- **Implementers do not run `make check`, `make mutate`, or push.** The controller runs the gates (Task 5). Implementers run only the targeted `go test` commands named in their task, plus `gofmt -w` on the files they touched (the code blocks below are written for readability, not for gofmt's alignment).
