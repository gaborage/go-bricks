...
- The registry re-declares its topology once per new channel of EVERY client the manager builds for its key — its own and each pooled publisher's, driven by the client's new-channel announcement as well as by a consumer re-subscribe — but always DECLARES through its own client; the publisher source is the one that matters when an operator deletes an exchange under a live connection, since only the publisher's channel takes the 404. The guard is keyed per `(source, generation)`, and a pooled publisher's entry is dropped when the pool retires it. Sources are only clients that are, or embed, the one `NewAMQPClient` returns; a declaration refused with `PRECONDITION_FAILED` is skipped until the process restarts (ADR-113).
