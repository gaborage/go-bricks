...
func (o *OracleContainer) NewSchema(t *testing.T) (*OracleSchema, context.Context) {
	...
	t.Cleanup(func() {
		...
		defer cancelDrop()
		// SECURITY: Manual SQL review completed - schemaName is generated by
		// randomSchemaName() from crypto/rand and constrained to GBTEST_[0-9A-F]{8};
		// never accepts caller-provided input. Oracle does not parameterize
		// identifiers in DDL.
		if _, err := adminDB.ExecContext(dropCtx,
			fmt.Sprintf("DROP USER %s CASCADE", schemaName)); err != nil {
	...
	//
	// SECURITY: Manual SQL review completed - schemaName comes from
	// randomSchemaName() (GBTEST_<8-hex>, crypto/rand); schemaPassword comes
	// from randomPassword() (24 hex chars, crypto/rand). Both constrained to
	// safe alphabets and never accept caller-provided input. Quoting the
	// password with "..." preserves case so go-ora's case-sensitive auth
	// (Oracle 12c+ default) matches what we stored.
	provisionSQL := fmt.Sprintf(`
		BEGIN
			EXECUTE IMMEDIATE 'CREATE USER %s IDENTIFIED BY "%s"';
			EXECUTE IMMEDIATE 'GRANT CONNECT, RESOURCE, CREATE TYPE, CREATE PROCEDURE, CREATE SEQUENCE TO %s';
			EXECUTE IMMEDIATE 'GRANT UNLIMITED TABLESPACE TO %s';
		END;
	`, schemaName, schemaPassword, schemaName, schemaName)
	if _, err := adminDB.ExecContext(ctx, provisionSQL); err != nil {
