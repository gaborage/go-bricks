...
func ParseTime(timeStr string) time.Time {
	...
	if err != nil {
		panic(fmt.Sprintf("scheduler: invalid time format '%s' - expected HH:MM (24-hour), error: %v", timeStr, err)) // NOSONAR: Fail-fast on invalid time format (configuration error)
	}
	return t
}
