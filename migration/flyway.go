...
const (
	...
	// flagURL carries the framework-built JDBC URL. A command-line flag outranks any
	// flyway.url in the operator's conf, which is the point: the migration target and its
	// TLS posture become the same database.* fields the runtime connects with (ADR-085).
	flagURL = "-url="
...
func (fm *FlywayMigrator) runFor(ctx context.Context, db *config.DatabaseConfig, cfg *Config, verb string) (Result, error) {
	...
	// A framework-built -url= outranks any flyway.url in the conf, so the migration
	// target and its TLS posture match the database.* fields the runtime uses (ADR-085).
	urlFlags, err := urlArgs(db, vendor, fm.appName())
	if err != nil {
		return Result{}, err
	}
...
// formatted into the Flyway subprocess environment OR into its argv (the
// framework-built -url=, see urlArgs) if it contains CR, LF, or NUL. Both sinks
// depend on this field list staying complete. The error names the offending
// field but never echoes the value (since it may be the password).
