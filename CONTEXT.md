...
governs waiting — connection readiness and the serialized publish slot — never
an in-flight socket write, so a broker that stops reading can hold one publish
past the bound until that write returns.
