...
**Escape hatch:** `f.Raw(...)`, `jf.Raw(...)`, `SetExpr(column, expr, args...)` on an UPDATE, every `RawExpression` SQL body (`qb.Expr()`, `qb.MustExpr()`, a struct literal), and `database.Raw(sql, args...)` (the Execute Helpers' hand-written-SQL adapter, broader than `f.Raw` — it replaces the whole statement, see [wiki/database.md#execute-helpers](../wiki/database.md#execute-helpers)) require a `// SECURITY: Manual SQL review completed - <rationale>` annotation at every call site; the authoritative rule (the `Having()` string-predicate case, the grep patterns) is the root [CLAUDE.md](../CLAUDE.md) Security Guidelines.
