...
// inputs is the role-agnostic view the shared helpers take.
func (k *ConsumerKeySources) inputs() keyInputs {
	return keyInputs{
		signFile:     k.SignFile,
		signValue:    k.SignValue,
		encryptFile:  k.EncryptFile,
		encryptValue: k.EncryptValue,
		signOptional: false, // an opener has no unsigned mode
	}
}
...
func (k *ConsumerKeySources) Load(signKid, encryptKid string) (*keymaterial.ConsumerKeys, error) {
	signPub, encPriv, err := loadKeyInputs(k.inputs(), keymaterial.LoadRSAPublicKey, keymaterial.LoadRSAPrivateKey)
	if err != nil {
		return nil, err
	}
