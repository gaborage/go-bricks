...
// ConsumerKeySources for an opener (sign PUBLIC, encrypt PRIVATE) — plus
// DocumentSpec, the kid-and-subject flags turned into a raw-document Spec. Each
...
func (k *KeySources) Validate() error {
	return validateKeyInputs(k.inputs())
}

// inputs is the role-agnostic view the shared helpers take. It is the only
// place the producer's sign-optionality reaches them.
func (k *KeySources) inputs() keyInputs {
	return keyInputs{
		signFile:     k.SignFile,
		signValue:    k.SignValue,
		encryptFile:  k.EncryptFile,
		encryptValue: k.EncryptValue,
		signOptional: k.SignOptional,
	}
}

// keyInputs is the four key sources plus their one policy bit: what both
// shared helpers read, in whichever role the caller holds them.
type keyInputs struct {
	signFile, signValue, encryptFile, encryptValue string
	signOptional                                   bool
}

// signAbsentAndOptional reports whether the caller may proceed unsigned and no
// sign source was given. Both helpers read it, so the conjunction has one home.
func (p keyInputs) signAbsentAndOptional() bool {
	return p.signOptional && p.signFile == "" && p.signValue == ""
}

// validateKeyInputs is the exactly-one-of-per-pair rule itself, shared with the
// consumer-role sources so both binaries refuse in one vocabulary.
func validateKeyInputs(p keyInputs) error {
	if !exactlyOne(p.signFile, p.signValue) && !p.signAbsentAndOptional() {
		return errors.New("exactly one of -sign-key-file or -sign-key-value is required")
	}
	if !exactlyOne(p.encryptFile, p.encryptValue) {
...
// — then loads and parses the keys and returns the producer-role resolver under
// the given kids. A SignOptional caller with no sign source gets a nil sign
// key. The refusals precede any I/O, so a mistyped invocation costs no file
// read.
func (k *KeySources) Load(signKid, encryptKid string) (*keymaterial.ProducerKeys, error) {
	signPriv, encPub, err := loadKeyInputs(k.inputs(), keymaterial.LoadRSAPrivateKey, keymaterial.LoadRSAPublicKey)
	if err != nil {
		return nil, err
	}
...
// loadKeyInputs is the loading half both roles run: validate, then load each source with
// the loader its role gives that half, under the "sign key: " / "encrypt key: " prefixes
// the CLIs report. The sign load is skipped, leaving the zero S, when the sign source is
// absent and optional.
func loadKeyInputs[S, E any](
	p keyInputs,
	...
	if vErr := validateKeyInputs(p); vErr != nil {
		return noSign, noEncrypt, vErr
	}
	if !p.signAbsentAndOptional() {
		if sign, err = loadSign(p.signFile, p.signValue); err != nil {
			return noSign, noEncrypt, fmt.Errorf("sign key: %w", err)
		}
	}
	if encrypt, err = loadEncrypt(p.encryptFile, p.encryptValue); err != nil {
